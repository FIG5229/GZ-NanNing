    ...
    <script>
        ...
        function formSubmit() {
            ...
            $('input[name="standardbox"]').each(function () {
                if ($(this).is(':checked')) {
                    selectedBoxes = $(this).val();
                }
            });
            let term='';


            selectedBoxes = $('input[name="standardbox"]:checked').val();

            //debugger;er;
            <%--let rowList = ${rowlistJson};--%>
            // let rowList = $.parseJSON();
            // let standardId = $("#standardId").val(rowListJson.cartlist);
            <%--window.parent.addBeta(selectedBoxes,rowList,standardId,'${workflowId}');--%>
            $.jBox.closeTip();
            /*选中的行在数据库中标记为选中*/
            /*$.ajax({
                type:"post",
                url: "${ctx}/exam/examWorkflowDatas/updateTerm",
                data:{standardId:standardId,id:'${workflowDatasId}',objId:'${objId}',fillPersonId:'${fillPersonId}',rowId:selectedBoxes},
            ...
            // let row = rowList[selectedBoxes]
            // console.log(row.rowNum+row)
            window.parent.setRowNum(selectedBoxes);
            window.parent.getOptionByStandardRowNum(selectedBoxes);
...
<body>
<br>
<form:form id="queryForm" action="${ctx}/exam/examWorkflow/selectTerm" method="post" class="form-horizontal">
...
<form:form id="inputForm" action="" method="post" class="form-horizontal">
    ...
    <input id="standardId" name="standardId" value="${standardId}" type="hidden"/>
    <input id="workflowDatasId" name="workflowDatasId" value="${workflowDatasId}" type="hidden"/>
                ...
                <table class="table table-striped table-bordered table-condensed" style="width: 100%;">
                    ...
                    <c:forEach items="${rowlist}" var="row"  varStatus="status">
                        <tr id="s_${row.rowNum}">
                            ...
                            <td>
                                <input class="i-checks" type="radio" name="standardbox" value="${row.rowNum}">
                            </td>
