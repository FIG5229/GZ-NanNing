    ...
    <div id="modalSysSelf">
        <div class="modal-custom-content">
            <div class="modal-custom-tb">
                <div style="color: red">(说明：红色字体为推送的考评项,蓝色字体为自动考评,绿色为审核人员修改，紫色为绩效办修改)</div>
                        ...
                        <c:if test="${'1'.equals(workflowData.isSelected)}">
                            ...
                            >
                               ...
                               <%-- <c:forEach items="${columnList}" var="examStandardTemplateItemDefine">
                                ...
                                <td>
                                    <textarea cols="3" rows="3"  name='datas[${status.index}].items' class="required">${workflowData.items}</textarea>
                                </td>
                                <td>
                                ...
                                <td><input type='text' name='datas[${status.index}].detail' style='width:50px;' value='${workflowData.detail}'
                                           <c:if test="${workflowData.isSelected == '3'}">disabled="disabled"</c:if> ></input></td>
                                ...
                                <td>
                                    <c:if test="${workflowData.adjustPersonId == fns:getUser().id}">

                                      <label><input class='i-checks' type='radio' name='datas[${status.index}].isAlter' value='2' <c:if test="${'2' == workflowData.isAlter}">checked</c:if>/>修改</label>
                                          <c:choose>
                                              <c:when test="${workflowData.isAlter != '2'}">
                                                  <label><input class='i-checks' type='radio' name='datas[${status.index}].isAlter' value='${workflowData.isAlter}' <c:if test="${'2' !=  workflowData.isAlter}">checked</c:if>/>不修改</label>
                                              </c:when>
                                              <c:otherwise>
                                                  <label><input class='i-checks' type='radio' name='datas[${status.index}].isAlter' value='0' <c:if test="${'2' !=  workflowData.isAlter}">checked</c:if>/>不修改</label>
                                              </c:otherwise>
                                          </c:choose>
                                    </c:if>
                ...
                <div style="text-align: right;">审核人：<input type="text" name="fill_person" value="${fns:getUser().name}" disabled></br></br>
                    <c:choose>
                        <c:when test="${'7'.equals(nextStep)&& !'true'.equals(history)}">
                            ...
                            <input id="complete" class="btn btn-primary" type="button" value="推送到下一环节" onclick="sendSubmit('groupAdjust','mainBureausSign')"   <c:if test="${'false'==isEdit}"> disabled="true" </c:if> />
                        ...
                        <c:when test="${'8'.equals(nextStep)&& !'true'.equals(history)}">
                            <input id="save" class="btn btn-primary" type="button" value="保存" onclick="formSubmit('save')"    <c:if test="${'false'==isEdit &&!'true'.equals(isSave)}"> disabled="true" </c:if>   />&nbsp;&nbsp;&nbsp;&nbsp;
                            <input id="complete" class="btn btn-primary" type="button" value="结束本项工作考评" onclick="formSubmit('mainBureausSignAll')"   <c:if test="${'false'==isEdit}"> disabled="true" </c:if>  />
                        </c:when>
                        ...
                        <c:otherwise>
                            <c:if test="${!'true'.equals(history)}">
                                <input id="complete" class="btn btn-primary" type="button" value="推送到下一环节" onclick="sendSubmit('mainBureausSignAll')"   <c:if test="${'false'==isEdit}"> disabled="true" </c:if> />
