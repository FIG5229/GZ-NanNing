...
<head>
    ...
    <script type="text/javascript">
        ...
        function selectTerm() {
            ...
            var url = "iframe:${ctx}/exam/examWorkflow/selectTerm?standardId=${standardInfoList.get(0).id}&workflowId=${workflowId}&objId=${objId}&fillPersonId=${fillPersonId}&status=2";
            </c:when>
            <c:otherwise>
            var url = "iframe:${ctx}/exam/examWorkflow/selectTerm?&workflowId=${workflowId}&objId=${objId}&fillPersonId=${fillPersonId}&status=2";
            </c:otherwise>
            </c:choose>
        ...
        /*接收记录*/
        function pushHistory(workflowDatasId,rowNum,pushHistoryId) {
            this.$.jBox.open("iframe:${ctx}/exam/examPushHistory/pushHistory?workflowId=${workflowId}&workflowDatasId="+workflowDatasId+"&rowNum="+rowNum+"&pushHistoryId="+pushHistoryId, "推送记录",width,height,{
                buttons:{"关闭":true},
                loaded:function(h){
                    $(".jbox-content", top.document).css("overflow-y","hidden");
                },closed:function (){
...
<form:form id="inputForm" action="${ctx}/exam/examWorkflowDatas/saveBeta" method="post" class="form-horizontal">
    ...
    <input id="isUpdateSorce" name="isUpdateSorce" value="${isSave}" type="hidden"/>
                ...
                <table class="table table-striped table-bordered table-condensed" style="width: 100%;">
                    ...
                    <tr>
                       ...
                       <%-- <c:choose>
                        ...
                        <c:if test="${processType == 'appraiseExam' && history != 'true'}">
                            <th>是否修改</th>
                            <th>操作</th>
                            <th>
                                <c:choose>
                                    <c:when test="${nextStep == '3' && nnextStep == '4'}">
                                        <%--选择每一条考评项的下一环节评审人--%>
                                        部门负责人签字环节审核人
                                    </c:when>
                                    <c:when test="${nextStep == '3'  && nnextStep == '5'}">
                                        <%--选择每一条考评项的下一环节评审人--%>
                                        分管局领导签字环节审核人
                                    </c:when>
                                    <c:when test="${nextStep == '3' && nnextStep == '6'}">
                                        <%--选择每一条考评项的下一环节评审人--%>
                                        绩效办调整环节审核人
                                    </c:when>
                                    <c:when test="${nextStep == '3' && nnextStep == '7'}">
                                        <%--选择每一条考评项的下一环节评审人--%>
                                        局领导签字环节审核人
                                    </c:when>
                    ...
                    <c:forEach items="${workflowDatasList}" var="workflowData"   varStatus="status">
                        <c:if test="${'1'.equals(workflowData.isSelected)}">
                            ...
                            <%--    <c:choose>
                                ...
                                <td><input type='text' name='datas[${status.index}].detail' style='width:50px;' value='${workflowData.detail}'></input></td>
                                </c:if>
                                <td>
                                    <c:choose>
                                ...
                                <c:if test="${processType == 'appraiseExam' && history != true}">
                                    <td>
                                    <c:if test="${workflowData.examPersonId == fns:getUser().id}">

                                        <label><input class='i-checks' type='radio' name='datas[${status.index}].isAlter' value='1' <c:if test="${'1' == workflowData.isAlter}">checked</c:if>/>修改</label>
                                        <label><input class='i-checks' type='radio' name='datas[${status.index}].isAlter' value='0' <c:if test="${'1' !=  workflowData.isAlter}">checked</c:if>/>不修改</label>
                                    </c:if>
                                    </td>
                                    <td>
                                        ...
                                        <br>
                                        <a href="javascript:;" onclick="pushHistory('${workflowData.id}','${workflowData.rowId}','${workflowData.pushHistoryId}')">推送记录</a>
                                        ...
                                        <c:choose>
                                            ...
                                            <c:when test="${nextStep == '3' && nnextStep == '4'}">
                                                ...
                                                <sys:treeselect id="datas${status.index}person" name="datas[${status.index}].departSignId" value="${workflowData.departSignId}" labelName="datas[${status.index}].departSign" labelValue="${workflowData.departSign}"
                                                                title="部门负责人签字" url="/sys/office/treeData?type=3" cssClass="input-small required" allowClear="true" notAllowSelectParent="true" isAll="true" cssStyle="required"/>
                                            </c:when>
                                            <c:when test="${nextStep == '3'  && nnextStep == '5'}">
                                                <%--选择每一条考评项的下一环节评审人--%>
                                                <sys:treeselect id="datas${status.index}person" name="datas[${status.index}].partBureausSignId" value="${workflowData.partBureausSignId}" labelName="datas[${status.index}].partBureausSign" labelValue="${workflowData.partBureausSign}"
                                                                title="分管局领导签字" url="/sys/office/treeData?type=3" cssClass="input-small required" allowClear="true" notAllowSelectParent="true" isAll="true"/>
                                            </c:when>
                                            <c:when test="${nextStep == '3' && nnextStep == '6'}">
                                                <%--选择每一条考评项的下一环节评审人--%>
                                                <sys:treeselect id="datas${status.index}person" name="datas[${status.index}].adjustPersonId" value="${workflowData.adjustPersonId}" labelName="datas[${status.index}].adjustPerson" labelValue="${workflowData.adjustPerson}"
                                                                title="	局绩效办调整" url="/sys/office/treeData?type=3" cssClass="input-small required" allowClear="true" notAllowSelectParent="true" isAll="true"/>
                                            </c:when>
                                            <c:when test="${nextStep == '3' && nnextStep == '7'}">
                                                <%--选择每一条考评项的下一环节评审人--%>
                                                <sys:treeselect id="datas${status.index}person" name="datas[${status.index}].mainBureausSignId" value="${workflowData.mainBureausSignId}" labelName="datas[${status.index}].mainBureausSign" labelValue="${workflowData.mainBureausSign}"
                                                                title="	局领导审签" url="/sys/office/treeData?type=3" cssClass="input-small required" allowClear="true" notAllowSelectParent="true" isAll="true"/>
                                            </c:when>
                                            <c:when test="${nextStep == '4'}">
                                                <%--选择每一条考评项的下一环节评审人--%>
                                                <sys:treeselect id="datas${status.index}person" name="datas[${status.index}].departSignId" value="${workflowData.departSignId}" labelName="datas[${status.index}].departSign" labelValue="${workflowData.departSign}"
                                                                title="部门负责人签字" url="/sys/office/treeData?type=3" cssClass="input-small required" allowClear="true" notAllowSelectParent="true" isAll="true"/>
                                            </c:when>
                                            <c:when test="${nextStep == '5'}">
                                                <%--选择每一条考评项的下一环节评审人--%>
                                                <sys:treeselect id="datas${status.index}person" name="datas[${status.index}].partBureausSignId" value="${workflowData.partBureausSignId}" labelName="datas[${status.index}].partBureausSign" labelValue="${workflowData.partBureausSign}"
...
<%--                            <c:set var="num" value="${num+step}"></c:set>--%>
                ...
                <c:if test="${history != 'true'}">
                    ...
                    <c:choose>
                        <c:when test="${'3'.equals(nextStep) && '4'.equals(nnextStep)&& !'true'.equals(history)||'true'.equals(isSave)}" >
                            <input id="save" class="btn btn-primary" type="button" value="保存" onclick="formSubmit('save')"  <c:if test="${'false'==isEdit&&!'true'.equals(isSave)}"> disabled="true" </c:if> />&nbsp;&nbsp;&nbsp;&nbsp;
                            <c:if test="${!'8'.equals(nextStep)}">
<%--                            <input id="complete" class="btn btn-primary" type="button" value="推送到下一环节" onclick="sendSubmit('appraiseExam','departSign')"  <c:if test="${'false'==isEdit}"> disabled="true" </c:if> />--%>
                        ...
                        <c:when test="${'3'.equals(nextStep) &&'5'.equals(nnextStep)&& !'true'.equals(history)||'true'.equals(isSave)}">
                            <input id="save" class="btn btn-primary" type="button" value="保存" onclick="formSubmit('save')"  <c:if test="${'false'==isEdit&&!'true'.equals(isSave)}"> disabled="true" </c:if> />&nbsp;&nbsp;&nbsp;&nbsp;
<%--                            <input id="complete" class="btn btn-primary" type="button" value="推送到下一环节" onclick="sendSubmit('appraiseExam','partBureausSign')"  <c:if test="${'false'==isEdit}"> disabled="true" </c:if> />--%>
                            <c:if test="${workflowDatasList.size() == 0}">
                                <sys:treeselect id="person" name="personId" value="${personId}" labelName="personName" labelValue="${person}"
                                                title="用户" url="/sys/office/treeData?type=3" cssClass="" allowClear="true" notAllowSelectParent="true" isAll="true"/>
                            </c:if>
                        </c:when>
                        <c:when test="${'3'.equals(nextStep) &&'6'.equals(nnextStep)&& !'true'.equals(history)||'true'.equals(isSave)}">
                            <input id="save" class="btn btn-primary" type="button" value="保存" onclick="formSubmit('save')"  <c:if test="${'false'==isEdit&&!'true'.equals(isSave)}"> disabled="true" </c:if> />&nbsp;&nbsp;&nbsp;&nbsp;
<%--                            <input id="complete" class="btn btn-primary" type="button" value="推送到下一环节" onclick="sendSubmit('appraiseExam','groupAdjust')"  <c:if test="${'false'==isEdit}"> disabled="true" </c:if> />--%>
                        ...
                        <c:when test="${'3'.equals(nextStep) &&'7'.equals(nnextStep)&& !'true'.equals(history)||'true'.equals(isSave)}">
                            <input id="save" class="btn btn-primary" type="button" value="保存" onclick="formSubmit('save')"  <c:if test="${'false'==isEdit&&!'true'.equals(isSave)}"> disabled="true" </c:if> />&nbsp;&nbsp;&nbsp;&nbsp;
                            <input id="complete" class="btn btn-primary" type="button" value="公示" onclick="sendSubmit('appraiseExam','mainBureausSign')"  <c:if test="${'false'==isEdit}"> disabled="true" </c:if> />
                            <c:if test="${workflowDatasList.size() == 0}">
                                <sys:treeselect id="person" name="personId" value="${personId}" labelName="personName" labelValue="${person}"
                                                title="用户" url="/sys/office/treeData?type=3" cssClass="" allowClear="true" notAllowSelectParent="true" isAll="true"/>
                            </c:if>
                        </c:when>
                        <c:when test="${'3'.equals(nextStep) &&'8'.equals(nnextStep)&& !'true'.equals(history)||'true'.equals(isSave)}">
                            <input id="complete" class="btn btn-primary" type="button" value="公示" onclick="formSubmit('appraiseExam','')"   <c:if test="${'false'==isEdit}"> disabled="true" </c:if> />
                        </c:when>
                        <c:when test="${'4'.equals(nextStep)&& !'true'.equals(history)||'true'.equals(isSave)}">
                            <input id="save" class="btn btn-primary" type="button" value="保存" onclick="formSubmit('save')"  <c:if test="${'false'==isEdit&&!'true'.equals(isSave)}"> disabled="true" </c:if> />&nbsp;&nbsp;&nbsp;&nbsp;
<%--                            <input id="complete" class="btn btn-primary" type="button" value="推送到下一环节" onclick="sendSubmit('appraiseExam','departSign')"  <c:if test="${'false'==isEdit}"> disabled="true" </c:if> />--%>
                            <c:if test="${workflowDatasList.size() == 0}">
                                <sys:treeselect id="person" name="personId" value="${personId}" labelName="personName" labelValue="${person}"
                        ...
                        <c:when test="${'5'.equals(nextStep)&& !'true'.equals(history)||'true'.equals(isSave)}">
                            <input id="save" class="btn btn-primary" type="button" value="保存" onclick="formSubmit('save')"  <c:if test="${'false'==isEdit&&!'true'.equals(isSave)}"> disabled="true" </c:if> />&nbsp;&nbsp;&nbsp;&nbsp;
<%--                            <input id="complete" class="btn btn-primary" type="button" value="推送到下一环节" onclick="sendSubmit('appraiseExam','partBureausSign')"  <c:if test="${'false'==isEdit}"> disabled="true" </c:if> />--%>
                            <c:if test="${workflowDatasList.size() == 0}">
                                <sys:treeselect id="person" name="personId" value="${personId}" labelName="personName" labelValue="${person}"
                        ...
                        <c:when test="${'6'.equals(nextStep)&& !'true'.equals(history)||'true'.equals(isSave)}">
                            <input id="save" class="btn btn-primary" type="button" value="保存" onclick="formSubmit('save')"  <c:if test="${'false'==isEdit&&!'true'.equals(isSave)}"> disabled="true" </c:if> />&nbsp;&nbsp;&nbsp;&nbsp;
<%--                            <input id="complete" class="btn btn-primary" type="button" value="推送到下一环节" onclick="sendSubmit('appraiseExam','groupAdjust')"  <c:if test="${'false'==isEdit}"> disabled="true" </c:if> />--%>
                            <c:if test="${workflowDatasList.size() == 0}">
                                <sys:treeselect id="person" name="personId" value="${personId}" labelName="personName" labelValue="${person}"
                        ...
                        <c:when test="${'7'.equals(nextStep)&& !'true'.equals(history)||'true'.equals(isSave)}">
                            <input id="save" class="btn btn-primary" type="button" value="保存" onclick="formSubmit('save')"  <c:if test="${'false'==isEdit&&!'true'.equals(isSave)}"> disabled="true" </c:if> />&nbsp;&nbsp;&nbsp;&nbsp;
<%--                            <input id="complete" class="btn btn-primary" type="button" value="推送到下一环节" onclick="sendSubmit('appraiseExam','mainBureausSign')"  <c:if test="${'false'==isEdit}"> disabled="true" </c:if> />--%>
                        ...
                        <c:when test="${'8'.equals(nextStep)&& !'true'.equals(history)||'true'.equals(isSave)}">
                            <input id="save" class="btn btn-primary" type="button" value="保存" onclick="formSubmit('save')"  <c:if test="${'false'==isEdit&&!'true'.equals(isSave)}"> disabled="true" </c:if>  />&nbsp;&nbsp;&nbsp;&nbsp;
<%--                            <input id="complete" class="btn btn-primary" type="button" value="公示 " onclick="formSubmit('appraiseExam','')" <c:if test="${status==nextStep-1}"> disabled="true" </c:if> />--%>
                        </c:when>
