        ...
        <div class="modal-custom-main">
            ...
            <div class="modal-custom-tab">
                <c:forEach items="${objList}" var="obj">
                    <div class="modal-custom-tab-item"><a href="${ctx}/exam/examWorkflow/exam?id=${workflowId}&objId=${obj.id}&status=${status}&processType=${processType}">${obj.name}(<fmt:formatNumber type="number" value="${100-obj.value}" pattern="0.00" maxFractionDigits="2"/>)</a></div>
                </c:forEach>
            </div>-->
            <table id="modalPeoTb" class="table table-striped table-bordered table-condensed">
                ...
                <tr>
                    <c:forEach items="${objList}" var="obj">
                        <td><a href="${ctx}/exam/examWorkflow/exam?id=${workflowId}&objId=${obj.id}&status=${status}&processType=${processType}" <c:if test="${objId==obj.id}"> style="color: red"</c:if>>${obj.name}(<fmt:formatNumber type="number" value="${100+obj.value}" pattern="0.00" maxFractionDigits="2"/>)</a></td>
                    </c:forEach>
                </tr>
                </tbody>
            </table>
