...
<head>
    ...
    <script type="text/javascript">
        ...
        function selectTerm() {
            ...
            var url = "iframe:${ctx}/exam/examWorkflow/selectTerm?standardId=${standardInfoList.get(0).id}&workflowId=${workflowId}&objId=${objId}"+
            "&fillPersonId=${fillPersonId}&status=2";
            </c:when>
            <c:otherwise>
        ...
        function recovery(id) {
            $.ajax({
                url: "${ctx}/exam/examWorkflowDatas/recoveryData",
                dataType: "json",
...
<%--                    <c:set var="step" value="1"></c:set>--%>
                    <c:forEach items="${workflowDatasList}" var="workflowData"   varStatus="status">
                        <c:if test="${!'0'.equals(workflowData.isSelected)}">
...
<%--                                <td>${workflowData.items}</td>--%>
                                ...
                                <c:if test="${workflowData.status == '3' || workflowData.status == '2'}">
                                    <td>
                                    ...
                                    <c:if test="${workflowData.isAlreadySelected =='99'|| workflowData.isAlreadySelected == '66' }">
                                        ...
                                        <c:if test="${workflowData.isSelected == '3'}">
                                            <a href="javascript:;" onclick="recovery('${workflowData.id}')">恢复</a>
                                        </c:if>
                                        ...
                                        <c:choose>
                                            ...
                                            <c:when test="${nextStep == '3' && nnextStep == '4'}">
                                                ...
                                                <sys:treeselect id="datas${status.index}person" name="datas[${status.index}].departSignId" value="${workflowData.departSignId}" labelName="datas[${status.index}].departSign" labelValue="${workflowData.departSign}"
                                                                title="用户" url="/sys/office/treeData?type=3" cssClass="input-small required" allowClear="true" notAllowSelectParent="true" isAll="true" cssStyle="required"/>
                                            </c:when>
                                            <c:when test="${nextStep == '3'  && nnextStep == '5'}">
                                                <%--选择每一条考评项的下一环节评审人--%>
                                                <sys:treeselect id="datas${status.index}person" name="datas[${status.index}].partBureausSignId" value="${workflowData.partBureausSignId}" labelName="datas[${status.index}].partBureausSign" labelValue="${workflowData.partBureausSign}"
                                                                title="用户" url="/sys/office/treeData?type=3" cssClass="input-small required" allowClear="true" notAllowSelectParent="true" isAll="true"/>
                                            </c:when>
                                            <c:when test="${nextStep == '3' && nnextStep == '6'}">
                                                <%--选择每一条考评项的下一环节评审人--%>
...
<%--                            <c:set var="num" value="${num+step}"></c:set>--%>
                    ...
                    <c:choose>
                        <%--初核的时候保存，系统公示的可以改分  都可以保存--%>
                        <c:when test="${status eq '2' || status eq '3'}">
                        ...
                        <c:when test="${'3'.equals(nextStep) &&'8'.equals(nnextStep)&& !'true'.equals(history)||'true'.equals(isSave)}">
                            <input id="complete" class="btn btn-primary" type="button" value="通过" onclick="formSubmit('appraiseExam','')"   <c:if test="${'false'==isEdit}"> disabled="true" </c:if> />
                        </c:when>
                        ...
                        <c:when test="${'8'.equals(nextStep)&& !'true'.equals(history)||'true'.equals(isSave)}">
                            <input id="save" class="btn btn-primary" type="button" value="保存" onclick="formSubmit('save')"  <c:if test="${'false'==isEdit&&!'true'.equals(isSave)}"> disabled="true" </c:if>  />&nbsp;&nbsp;&nbsp;&nbsp;
                            <input id="complete" class="btn btn-primary" type="button" value="通过" onclick="formSubmit('appraiseExam','')" <c:if test="${status==nextStep-1}"> disabled="true" </c:if> />
                        </c:when>
