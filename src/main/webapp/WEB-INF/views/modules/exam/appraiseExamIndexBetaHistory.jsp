...
<script type="text/javascript">
	...
	function endExam(){
		...
		if (nNextStatus == '99'){
			...
			top.$.jBox.confirm("请先保存选择的考评项，确认要结束选中的考核吗", '系统提示', function (v, h, f) {
				if (v == 'ok') {
					$.getJSON("${ctx}/exam/examWorkflowDatas/saveByIdsBetaHistory",
							{
								"fillPersonIds": array,
								"status":"${status}",
	...
	function backExam() {
		...
		top.$.jBox.confirm("确认要退回选中的考核吗", '系统提示', function (v, h, f) {
			...
			if (v == 'ok') {
				$.post("${ctx}/exam/examWorkflowDatas/saveByIdsBetaHistory",
						{
							"fillPersonIds": array,
							"standardId": "${standardId}",
							"workflowId": '${workflowId}',
	...
	var setting = {
		...
		callback:{onClick:function(event, treeId, treeNode){
				...
				if (!treeNode.isParent) {
					...
					$('#applicantContent').attr("src","${ctx}/exam/examWorkflowDatas/examBetaById?examType=${examType}&fillPersonId="+id+"&standardId=${standardId}"+
					"&examWorkflowId=${workflowId}&status=${status}&objName=${objName}&history=${history}&processType=${processType}&workflowType="+workflowType+
					"&personType=${personType}&isEdit="+$("#isEdit").val());
				}else {
