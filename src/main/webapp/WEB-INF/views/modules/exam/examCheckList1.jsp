...
<head>
	...
	<script type="text/javascript">
		...
		function openAddEditDialog(id) {
			var url = "iframe:${ctx}/exam/examCheck1/form?id="+id;
			if (id == null || id == undefined){
				url = "iframe:${ctx}/exam/examCheck1/form";
			}
			top.$.jBox.open(url, "问题整改添加",1000,650,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){
					self.location.href="${ctx}/exam/examCheck1/";
				}
			});
		};
		function openDialog(id) {
			top.$.jBox.open("iframe:${ctx}/exam/examCheck1/shenHeDialog?id="+id, "审核",800,420,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},
				closed:function (){
					self.location.href="${ctx}/exam/examCheck1/manageList";
				}
			});
		};
		//详情弹窗
...
<%--		<li><a href="${ctx}/exam/examCheck1/">查询</a></li>--%>
<%--		<shiro:hasPermission name="exam:examCheck1:manage"><li class="active"><a href="${ctx}/exam/examCheck1/manageList">管理</a></li></shiro:hasPermission>--%>
	<c:choose>
		<c:when test="${mType==null}">
			...
			<shiro:hasPermission name="exam:examCheck1:manage">
				<li  class="active"><a href="${ctx}/exam/examCheck1/manageList">问题整改管理</a></li>
			</shiro:hasPermission>
...
<%--			</shiro:hasPermission>--%>
	...
	<form:form id="searchForm" modelAttribute="examCheck1" action="${ctx}/exam/examCheck1/manageList" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form2">
			...
			<shiro:hasPermission name="exam:examCheck1:edit">
				<%--<li class="btns"><input  class="btn btn-primary"  type="button" value="添加" onclick="openAddEditDialog('${ctx}/exam/examCheck1/form')"/></li>
                <li class="btns"><input id="del" class="btn btn-primary" type="button" value="删除" onclick="deleteByIds('${ctx}/exam/examCheck1/deleteByIds','checkAll','myCheckBox')"/></li>--%>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="examCheck1" varStatus="status">
			<tr>
				...
				<shiro:hasPermission name="exam:examCheck1:manage"><td class="handleTd">
					<a onclick="openDialog('${examCheck1.id}')">审核</a>
    				<%--<a href="javascript:void(0)" onclick="openAddEditDialog('${ctx}/exam/examCheck1/form?id=${examCheck1.id}')">修改</a>
					<a href="${ctx}/exam/examCheck1/delete?id=${examCheck1.id}" onclick="return confirmx('确认要删除该检查法相情况录入吗？', this.href)">删除</a>--%>
