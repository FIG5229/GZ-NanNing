...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/exam/examWorkflow/flowList?examType=${examType}">待办</a></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${list}" var="examWorkflow">
			<c:if test="${examWorkflow.status <=examWorkflow.personStatus||(examWorkflow.status == 3 && examWorkflow.personStatus==1)}">
			<tr>
				...
				<td>
					<c:choose>
						...
						<%--被考评对象查看时--%>
						<c:when test="${examWorkflow.noTree == 'true'}">
							<%--被考评对象的操作状态等于考评状态 查看后操作--%>
							<c:if test="${examWorkflow.personStatus == examWorkflow.status}">
								<a href="${ctx}/exam/examWorkflowDatas/examBeta?examWorkflowId=${examWorkflow.id}&examType=${examType}&noTree=true"
								   style="color: red">查看</a>
							</c:if>
							<%--被考评对象的操作状态不等于考评状态  查看历史--%>
							<c:if test="${examWorkflow.personStatus != examWorkflow.status}">
								<a href="${ctx}/exam/examWorkflowDatas/examBeta?examWorkflowId=${examWorkflow.id}&examType=${examType}&history=true">查看</a>
							</c:if>
						</c:when>

						<c:when test="${examWorkflow.personStatus == examWorkflow.status}">
							<%--乱的不知道怎么改了 从头来--%>
							<c:if test="${examWorkflow.status == '1'}">
								<a href="${ctx}/exam/examWorkflowDatas/examBeta?examWorkflowId=${examWorkflow.id}&examType=${examType}" style="color: red">查看</a>
							</c:if>
							<c:if test="${examWorkflow.status != '1'}">
								<a href="${ctx}/exam/examWorkflowDatas/appraise/examIndex?examWorkflowId=${examWorkflow.id}&examType=${examType}" style="color: red">查看</a>
							</c:if>
<%--							<a href="${ctx}/exam/examWorkflow/examBeta?id=${examWorkflow.id}&examType=${examType}" style="color: red">查看</a>--%>
<%--							<a href="${ctx}/exam/examWorkflow/exam?id=${examWorkflow.id}&examType=${examType}" style="color: red">查看</a>--%>
						</c:when>
						<c:when test="${examWorkflow.personStatus==1 && examWorkflow.status >1}">
							<a href="${ctx}/exam/examWorkflowDatas/appraise/examIndex?examWorkflowId=${examWorkflow.id}&history=true&examType=${examType}" >查看</a>
						</c:when>
						<c:when test="${examWorkflow.personStatus==2 && examWorkflow.status <2}">
<%--							<a href="${ctx}/exam/examWorkflow/history?id=${examWorkflow.id}&preview=true">查看</a>--%>
							<a href="${ctx}/exam/examWorkflowDatas/appraise/examIndex?examWorkflowId=${examWorkflow.id}&preview=true&history=true&examType=${examType}">查看</a>
						</c:when>
						<c:when test="${examWorkflow.personStatus==2 && examWorkflow.status ==3}">
<%--							<a href="${ctx}/exam/examWorkflow/history?id=${examWorkflow.id}">查看</a>--%>
							<a href="${ctx}/exam/examWorkflowDatas/appraise/examIndex?examWorkflowId=${examWorkflow.id}&history=true&examType=${examType}">查看</a>
						</c:when>
						<c:when test="${examWorkflow.personStatus==6 && examWorkflow.status <6}">
<%--							<a href="${ctx}/exam/examWorkflow/history?id=${examWorkflow.id}&preview=true">查看</a>--%>
<%--							<a href="${ctx}/exam/examWorkflowDatas/history?examWorkflowId=${examWorkflow.id}&preview=true">查看11</a>&ndash;%&gt;--%>
							<a href="${ctx}/exam/examWorkflowDatas/appraise/examIndex?examWorkflowId=${examWorkflow.id}&examType=${examType}&history=true">查看</a>
						</c:when>
						<c:otherwise>
						</c:otherwise>
					</c:choose>
