                    ...
                    <c:forEach items="${workflowDatasList}" var="workflowData"   varStatus="status">
                        <c:if test="${'1'.equals(workflowData.isSelected)}">
                            ...
                            >
                                ...
                                <%--<c:forEach items="${columnList}" var="examStandardTemplateItemDefine">
                                    <td>
                                            ${workflowData.get(String.valueOf(examStandardTemplateItemDefine.id))}
                                    </td>
                                </c:forEach>--%>
                                    ...
                                    <c:choose>
                                        <c:when test="${nextStep == '4'}">
                                            ...
                                            <sys:treeselect id="datas${status.index}person" name="datas[${status.index}].departSignId" value="${workflowData.departSignId}" labelName="datas[${status.index}].departSign" labelValue="${workflowData.departSign}"
                                                            title="用户" url="/sys/office/treeData?type=3" cssClass="required" allowClear="true" notAllowSelectParent="true" isAll="true" dataMsgRequired="必填信息"/>
                                        </c:when>
                                        <c:when test="${nextStep == '5'}">
                                            <%--选择每一条考评项的下一环节评审人--%>
                ...
                <c:if test="${history != 'true'}">
                    ...
                    <c:choose>
                        <c:when test="${'5'.equals(nextStep)}">
                            ...
                            <input id="complete" class="btn btn-primary" type="button" value="推送到下一环节" onclick="sendSubmit('departSign','partBureausSign')"   <c:if test="${'false'==isEdit}"> disabled="true" </c:if> />
