...
<head>
    ...
    <script type="text/javascript">
        ...
        function selectTerm() {
            var url = "iframe:${ctx}/exam/examWorkflow/selectTerm?standardId=${standardId}&workflowId=${workflowId}&objId=${objId}&fillPersonId=${fillPersonId}";
        ...
        array.push("${columnList}");
        </c:forEach>
        function addBeta(selectedBoxes) {
            ...
            array.push("${columnList}");
                ...
                for (let index = 0; index < selectedBoxes.length; index++) {
                    ...
                    html += "<td>未提交</td>\n";
                    html += "<td><label><input class='i-checks' type='radio' name='datas[" + rowNum + "].operationStatus' value='2'/>不通过</label>\n" +
                        "<label><input class='i-checks' type='radio' name='datas[" + rowNum + "].operationStatus' value='3' checked/>通过</label></td>";
                    ...
                    <c:forEach items="${rowlist}" var="row"   varStatus="status">
                        <c:if test="${'1'.equals(row.isSelected)}">
                            <tr id="r_${row.row_num}">
                                ...
                                <td>
                                    <label><input class='i-checks' type='radio' name='datas[${num}].operationStatus' value='2' <c:if test="${'2'.equals(row.operationStatus)}">checked</c:if>/>不通过</label>
                                    <label><input class='i-checks' type='radio' name='datas[${num}].operationStatus' value='3' <c:if test="${!'2'.equals(row.operationStatus)}">checked</c:if>/>通过</label></td>
                ...
                <div style="text-align: right;">审核人：<input type="text" name="fill_person" value="${fns:getUser().name}" disabled></br></br>
                    <c:choose>
                        <c:when test="${'3'.equals(nextStep) && '4'.equals(nnextStep)&& !'true'.equals(history)||'true'.equals(isSave)}" >
                            ...
                            <input id="complete" class="btn btn-primary" type="button" value="公示并推送到" onclick="sendSubmit('appraiseExam','departSign')"  <c:if test="${'false'==isEdit}"> disabled="true" </c:if> />
...
<%--                            </c:if>--%>
              ...
              <div  style="text-align: right;">
                  <span class="tb-btn tb-jia"><i class="icon-plus-sign" onclick="selectTerm()"></i></span>
                  <span class="tb-btn tb-jian"><i class="icon-minus-sign" onclick="reduce()"></i></span>
              </div>
        </div>
    </div>
</form:form>
