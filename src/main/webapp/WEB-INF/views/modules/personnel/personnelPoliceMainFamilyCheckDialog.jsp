...
<body>
<form:form id="inputForm" modelAttribute="personnelPoliceMainFamily" action="${ctx}/personnel/personnelPoliceMainFamily/shenHe" method="post" class="form-horizontal" cssStyle="margin-top: 20px;">
    <form:hidden path="id"/>
...
&lt;%&ndash;            <form:options items="${fns:getDictList('affair_mein_shenhe')}" itemLabel="label" itemValue="value" htmlEscape="false"/>&ndash;%&gt;
        ...
        <input id="btnSubmit1" class="btn btn-primary" type="button" value="转送上一级" onclick="setCheckType(6)"/>&nbsp;
    ...
    <div style="margin-left: 500px;margin-top: 70px;">
        ...
        <input id="btnSubmit" class="btn btn-primary" type="button" value="审核通过" onclick="setCheckType(3)"/>&nbsp;
        </c:if>
        <c:if test="${personnelPoliceMainFamily.juChuCheckId eq fns:getUser().id}">
        ...
        <input id="btnSubmit2" class="btn btn-default" type="button" value="退回整改" onclick="setCheckType(5)"/>&nbsp;
    </div>
</form:form>
</body>
</html>