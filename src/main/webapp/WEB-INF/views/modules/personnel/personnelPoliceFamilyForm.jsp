...
<head>
	...
	<script type="text/javascript">
		...
		function complete() {
			$("#inputForm").attr("action","${ctx}/personnel/personnelPoliceFamily/save?isComplete=true");
			$("#inputForm").submit();
		}
...
<body>
	...
	<form:form id="inputForm" modelAttribute="personnelPoliceFamily" action="${ctx}/personnel/personnelPoliceFamily/save?add=${add}" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<input type="hidden" id="step" name="step" value="${personnelPoliceFamily.step}"/>
		...
		<div class="control-group">
			<label class="control-label">
				<c:choose>
					<c:when test="${personnelPoliceFamily.step == '0'}">
						父亲
					</c:when>
					<c:when test="${personnelPoliceFamily.step eq '1'}">
						母亲
					</c:when>
					<c:when test="${personnelPoliceFamily.step eq '2'}">
						配偶
					</c:when>
					<c:when test="${personnelPoliceFamily.step eq '3'}">
						配偶父亲
					</c:when>
					<c:when test="${personnelPoliceFamily.step eq '4'}">
						配偶母亲
					</c:when>
					<c:when test="${personnelPoliceFamily.step eq '5'}">
						兄弟姐妹
					</c:when>
					<c:when test="${personnelPoliceFamily.step eq '6'}">
						子女
					</c:when>
					<c:when test="${personnelPoliceFamily.step eq '7'}">
						子女配偶
					</c:when>
