...
<div id="content" class="row-fluid">
    ...
    <div id="right" style="margin-left: 60px;margin-top: 400px;">
        <form:form id="inputForm" modelAttribute="personnelBase" action="${ctx}/personnel/personnelBase/editWorkUnit?ids=${editPersonIds}"
                   method="post" class="form-horizontal">
            <input id="editPersonIds" name="editPersonIds" type="hidden" value="${editPersonIds}"/>
            <input id="editWorkUnitName" name="editWorkUnitName" type="hidden"/>
            <input id="editWorkUnitId" name="editWorkUnitId" type="hidden"/>
        </form:form>
...
<script type="text/javascript">
    var setting = {
        ...
        callback: {
            onClick: function (event, treeId, treeNode) {
                ...
                $("#editWorkUnitId").val(id)
                $("#editWorkUnitName").val(treeNode.name)
            }
        }
    };
    ...
    function save() {
        if ($("#editWorkUnitId").val() != '' && $("#editWorkUnitId").val() != undefined) {
            $("#inputForm").submit();
        } else {
            $.jBox.tip('请先选择单位');
        }
