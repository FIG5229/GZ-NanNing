...
<%--		<li><a href="${ctx}/personnel/personnelPoliceRank/?mType=${mType}">警衔信息</a></li>--%>
		...
		<li><a href="${ctx}/personnel/personnelPoliceRank/analysis?mType=${mType}">统计分析</a></li>
				...
				<form:form id="searchForm" modelAttribute="personnelPoliceRank" action="${ctx}/personnel/personnelPoliceRank/calculate" method="post" class="breadcrumb form-search">
...
<%--				<shiro:hasPermission name="personnel:personnelPoliceRank:edit"><th id="handleTh">操作</th></shiro:hasPermission>--%>
		...
		<tbody>
		<c:forEach items="${list}" var="personnnel" varStatus="status">
			<tr>
				...
				<td>
						${personnnel.get('base').name}

				</td>
				<td>
						${fns:getDictLabel(personnnel.get('base').sex, 'sex', '')}
				</td>
				<td>
					<fmt:formatDate value="${personnnel.get('base').birthday}" pattern="yyyy-MM-dd"/>
				</td>
				<td>
					<fmt:formatDate value="${personnnel.get('base').publicSecurityDate}" pattern="yyyy-MM-dd"/>
				</td>
				<td>
						${personnnel.get('base').jxyjxznx}
				</td>
				<td>
						${personnnel.get('base').jobFullname}
				</td>
				<td>
						${fns:getDictLabel(personnnel.get('job').jobLevel,"personnel_zwcc","")}
				</td>
				<td>
					<fmt:formatDate value="${personnnel.get('job').startDate}" pattern="yyyy-MM-dd"/>
				</td>
				<td>
						${personnnel.get('rank').changeReason}
				</td>
				<td>
						${fns:getDictLabel(personnnel.get('rank').policeRankLevel, "police_rank_level", "")}
				</td>
				<td>
					<fmt:formatDate value="${personnnel.get('rank').startDate}" pattern="yyyy-MM-dd"/>
				</td>
				<td>
						${fns:getDictLabel(personnnel.get('rank').policeRankLevel+1, "police_rank_level", "")}
				</td>
				<td>
					<fmt:formatDate value="${personnnel.get('rank').endDate}" pattern="yyyy-MM-dd"/>
				</td>
				<td>
