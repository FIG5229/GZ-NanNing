...
<head>
    ...
    <script type="text/javascript">
        ...
        function openEditDialog(id) {
            var url = "iframe:${ctx}/cms/article/viewForm?id="+id;
            top.$.jBox.open(url, "信息审核修改",1000,600,{
                buttons:{"关闭":true},
                loaded:function(h){
                    $(".jbox-content", top.document).css("overflow-y","hidden");
                },closed:function (){self.location.href="${ctx}/cms/article/view"}
            });
        };
...
<ul class="nav nav-tabs">
    <li class="active"><a href="${ctx}/cms/article/view?delFlag=2&category.id=${article.category.id}">信息审核</a></li>
...
<form:form id="searchForm" modelAttribute="article" action="${ctx}/cms/article/view" method="post"
    ...
    <label>状态：</label><form:radiobuttons onclick="$('#searchForm').submit();" path="delFlag"
                                         items="${fns:getDictList('cms_examine')}" itemLabel="label" itemValue="value"
                                         htmlEscape="false"/>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
    ...
    <c:forEach items="${page.list}" var="article" varStatus="status">
        <tr>
            ...
            <td>
<%--                <a href="${ctx}/cms/article/form?id=${article.id}" title="${article.title}">--%>
...
<%--                </a>--%>
            ...
            <td>
                ...
                <shiro:hasPermission name="cms:article:edit">
                    ...
                    <a href="javascript:;" onclick="openEditDialog('${article.id}')">修改</a>
                    <shiro:hasPermission name="cms:article:audit">
                        ...
                        <a href="${ctx}/cms/article/delete?id=${article.id}&delFlag=1&categoryId=${article.category.id}&isColumnAudi=true"
                           onclick="return confirmx('确认要删除该文章吗？', this.href)">删除</a>
                        <%--<a href="${ctx}/cms/article/delete?id=${article.id}&delFlag=${article.delFlag ne 0?'&isRe=true':''}&categoryId=${article.category.id}&isColumnAudi=true&delFlag=${delFlag}"
