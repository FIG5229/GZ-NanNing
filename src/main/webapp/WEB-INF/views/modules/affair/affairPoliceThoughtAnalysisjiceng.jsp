...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#btnExport").click(
					function(){
						var submit = function (v, h, f) {
							if (v == 'all') {
								$("#searchForm").attr("action","${ctx}/affair/affairPoliceThoughtAnalysis/export?classify=1");
								$("#searchForm").submit();
								$("#searchForm").attr("action","${ctx}/affair/affairPoliceThoughtAnalysis/jiceng}");
							}
							if (v == 'part') {
								$("#searchForm").attr("action","${ctx}/affair/affairPoliceThoughtAnalysis/export?flag=true&classify=1");
								$("#searchForm").submit();
								$("#searchForm").attr("action","${ctx}/affair/affairPoliceThoughtAnalysis/jiceng");
							}
							if (v == 'cancel') {
			...
			$("#btnImport").click(function(){
				top.$.jBox.open("iframe:${ctx}/file/template/download/view?id=affair_affairPoliceThoughtAnalysisJiCeng", "导入",800,520,{itle:"导入数据", buttons:{"关闭":true},
					bottomText:"导入文件不能超过5M，仅允许导入“xls”或“xlsx”格式文件！",closed:function (){self.location.href="${ctx}/affair/affairPoliceThoughtAnalysis/jiceng"}});
			});
			$("#btnAdd").click(function(){
				top.$.jBox.open("iframe:${ctx}/affair/affairPoliceThoughtAnalysis/jiCengForm?classify=1", "添加",800,520,{itle:"添加", buttons:{"关闭":true}
		...
		function openDetailDialog(id) {
			var url = "iframe:${ctx}/affair/affairPoliceThoughtAnalysis/jiCengformDetail?id="+id;
		...
		function openEditDialog(id) {
			var url = "iframe:${ctx}/affair/affairPoliceThoughtAnalysis/jiCengForm?id="+id;
...
<ul class="nav nav-tabs">
	...
	<c:if test="${fns:getUser().company.id =='34'||fns:getUser().company.id =='95'||fns:getUser().company.id =='156'}">
		<li class="active"><a href="${ctx}/affair/affairPoliceThoughtAnalysis/jiceng">基层党支部</a></li>
...
<form:form id="searchForm" modelAttribute="affairPoliceThoughtAnalysis" action="${ctx}/affair/affairPoliceThoughtAnalysis/jiceng" method="post" class="breadcrumb form-search">
...
<%--		<th>签收时间</th>--%>
	...
	<c:forEach items="${page.list}" var="affairPoliceThoughtAnalysis" varStatus="status">
		<tr>
			...
			<td>
					${fns:getDictLabel(affairPoliceThoughtAnalysis.reportType, 'warn_month', '')}
			</td>
<%--			<td>--%>
