...
<head>
	...
	<script type="text/javascript">
		...
		function openAddEditDialog(id) {
			var url = "iframe:${ctx}/affair/affairFileNotice/form?id="+id+"&flag=2";
			if (id == null || id == undefined){
				url = "iframe:${ctx}/affair/affairFileNotice/form?flag=2";
			}
			top.$.jBox.open(url, "工会通知通报",1000,800,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){
					self.location.href="${ctx}/affair/affairFileNotice/manageList?flag=2";
				}
			});
		};
		//详情弹窗
		function openDetailDialog(id) {
			var url = "iframe:${ctx}/affair/affairFileNotice/formDetail?id="+id+"&flag=2";
			top.$.jBox.open(url, "工会通知通报",1200,800,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				}
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/affair/affairFileNotice?flag=2">查询</a></li>
		<shiro:hasPermission name="affair:affairFileNotice:manage2"><li class="active"><a href="${ctx}/affair/affairFileNotice/manageList?flag=2">管理</a></li></shiro:hasPermission>
	</ul>
		...
		<ul class="ul-form">
            ...
            <li><label>通知类别：</label>
				<form:select id="typeSelect" path="type" class="input-medium">
                    ...
                    <form:options items="${fns:getDictList('affair_gh_notice_type')}" itemLabel="label"
			...
			<shiro:hasPermission name="affair:affairFileNotice:manage2">
			...
			<li class="btns"><input  class="btn btn-primary" type="button" value="重置" onclick="window.location.href='${ctx}/affair/affairFileNotice/manageList?flag=2'"/></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="affairFileNotice" varStatus="status">
			<tr>
                ...
                <td>
                        ${fns:getDictLabel(affairFileNotice.type, 'affair_gh_notice_type', '')}
                </td>
				<td>
				...
				<shiro:hasPermission name="affair:affairFileNotice:manage"><td class="handleTd">朱其贵朱其贵朱其贵朱其贵
					<c:if test="${affairFileNotice.createBy.id == fns:getUser().id}">
						...
						<a href="${ctx}/affair/affairFileNotice/delete?id=${affairFileNotice.id}&flag=2"
