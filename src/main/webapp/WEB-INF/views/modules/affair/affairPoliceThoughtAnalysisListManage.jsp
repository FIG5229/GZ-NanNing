	...
	<script type="text/javascript">
		$(document).ready(function() {
            ...
            $("#btnImport").click(function(){
                top.$.jBox.open("iframe:${ctx}/file/template/download/view?id=affair_affairPoliceThoughtAnalysis", "导入",800,520,{itle:"导入数据", buttons:{"关闭":true},
                    bottomText:"导入文件不能超过5M，仅允许导入“xls”或“xlsx”格式文件！",closed:function (){self.location.href="${ctx}/affair/affairPoliceThoughtAnalysis/manage"}});
            });
            $("#btnAdd").click(function(){
                top.$.jBox.open("iframe:${ctx}/affair/affairPoliceThoughtAnalysis/form", "添加",800,520,{itle:"添加", buttons:{"关闭":true}
                    ,closed:function (){
                        self.location.href="${ctx}/affair/affairPoliceThoughtAnalysis/manage";
                    }
                });
        ...
        function openEditDialog(id) {
            ...
            top.$.jBox.open(url, "民警动态思想分析详情",1000,600,{
                ...
                loaded:function(h){
                    $(".jbox-content", top.document).css("overflow-y","hidden");
                },closed:function (){self.location.href="${ctx}/affair/affairPoliceThoughtAnalysis/manage"}
            });
		};
...
<body>
	<ul class="nav nav-tabs">
		...
		<c:choose>
			<c:when test="${userId=='66937439b2124f328d1521968fab06db' || userId == '8a6819768aef40968e8f289842613276'} ">
				<li><a href="${ctx}/affair/affairPoliceThoughtAnalysis/list">公安处党委</a></li>
			</c:when>
			<c:when test="${userId == 'e3ac8381fb3247e0b64fd6e3c48bddc1' || userId=='d30e324c8f73492d9b74103374fbc689'|| userId=='d154234ecb35470e84fb95e53726866b'}">
				<li><a href="${ctx}/affair/affairPoliceThoughtAnalysis/list">基层党支部</a></li>
				<li class="active"><a href="${ctx}/affair/affairPoliceThoughtAnalysis/manage">公安处党委</a></li>
			</c:when>
			<c:otherwise>
				<li class="active"><a href="${ctx}/affair/affairPoliceThoughtAnalysis/manage">基层党支部</a></li>
				<%--其他账号进行上报--%>
			</c:otherwise>
		</c:choose>

		<%--<shiro:hasPermission name="affair:affairPoliceThoughtAnalysis:browse">
			<li><a href="${ctx}/affair/affairPoliceThoughtAnalysis/">
		...
		<shiro:hasPermission name="affair:affairPoliceThoughtAnalysis:manage">
			<li class="active"><a href="${ctx}/affair/affairPoliceThoughtAnalysis/manage">
					&lt;%&ndash; <c:choose>
	...
	<form:form id="searchForm" modelAttribute="affairPoliceThoughtAnalysis" action="${ctx}/affair/affairPoliceThoughtAnalysis/manage" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<input id="fileName" name="fileName" type="hidden" value="思想分析.xlsx"/>
		...
		<ul class="ul-form2">
			...
			<li class="btns"><input id="btnDel" class="btn btn-primary" type="button" value="删除"
                                    onclick="deleteByIds('${ctx}/affair/affairPoliceThoughtAnalysis/deleteByIds','checkAll','myCheckBox')"/></li>
			<li class="btns"><input id="btnReset" class="btn btn-primary" type="button" value="重置"
                                    onclick="window.location.href='${ctx}/affair/affairPoliceThoughtAnalysis/manage'"/></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="affairPoliceThoughtAnalysis" varStatus="status">
			<tr>
				...
				<shiro:hasPermission name="affair:affairPoliceThoughtAnalysis:edit"><td class="handleTd">
					...
					<a href="${ctx}/affair/affairPoliceThoughtAnalysis/report?id=${affairPoliceThoughtAnalysis.id}" onclick="return confirmx('确认要上报该民警思想动态分析吗？', this.href)">上报</a>
					<a href="${ctx}/affair/affairPoliceThoughtAnalysis/delete?id=${affairPoliceThoughtAnalysis.id}" onclick="return confirmx('确认要删除该民警思想动态分析吗？', this.href)">删除</a>
