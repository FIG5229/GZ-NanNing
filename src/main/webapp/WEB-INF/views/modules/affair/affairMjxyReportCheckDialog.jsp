...
<body>
<form:form id="inputForm" modelAttribute="affairMjxyReport" action="${ctx}/affair/affairMjxyReport/shenHe" method="post" class="form-horizontal" cssStyle="margin-top: 20px;">
    <form:hidden path="id"/>
    ...
    <c:choose>
        <%--上报给一级审核单位 一级审核完后上报给二级  二级进行通
        当审核单位为局里时 一定显示通过按钮--%>
        <c:when test="${fns:getUser().id.equals('b91d9ac0c32847c4ab6f21e910959198')}">
            <div style="margin-left: 500px;margin-top: 70px;">
                <input id="btnSubmit5" class="btn btn-primary" type="button" value="通过并结束" onclick="setCheckType(4)"/>&nbsp;
                <input id="btnSubmit4" class="btn btn-default" type="button" value="不通过" onclick="setCheckType(0)"/>&nbsp;
            </div>
        </c:when>
        <c:when test="${fns:getUser().id.equals(affairMjxyReport.oneCheckId)&&!fns:getUser().id.equals(affairMjxyReport.twoCheckId)}">
            <div class="controls">
                <label class="control-label">转送上一级（选填）：</label>
           <%--     <sys:treeselect id="twoCheckMan" name="twoCheckId" value="${affairMjxyReport.twoCheckId}" labelName="twoCheckMan" labelValue="${affairMjxyReport.twoCheckMan}"
                                title="送审单位" url="/sys/office/treeData?type=3" cssClass="" isAll="true" allowClear="true" notAllowSelectParent="true"/>
       --%>
        ...
        <c:when test="${fns:getUser().id.equals(affairMjxyReport.twoCheckId)}">
            <div style="margin-left: 500px;margin-top: 70px;">
