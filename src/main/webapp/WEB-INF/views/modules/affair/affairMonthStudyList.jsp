	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#export").click(
					function(){
						var submit = function (v, h, f) {
							if (v == 'all') {
								$("#searchForm").attr("action","${ctx}/affair/affairMonthStudy/export?treeId=${treeId}");
								$("#searchForm").submit();
								$("#searchForm").attr("action","${ctx}/affair/affairMonthStudy/list?treeId=${treeId}");
							}
							if (v == 'part') {
								$("#searchForm").attr("action","${ctx}/affair/affairMonthStudy/export?flag=true&treeId=${treeId}");
								$("#searchForm").submit();
								$("#searchForm").attr("action","${ctx}/affair/affairMonthStudy/list?treeId=${treeId}");
							}
							if (v == 'cancel') {
			...
			$("#btnImport").click(function(){
				top.$.jBox.open("iframe:${ctx}/file/template/download/view?id=affair_affairMonthStudy", "导入",800,520,{itle:"导入数据", buttons:{"关闭":true},
					bottomText:"导入文件不能超过5M，仅允许导入“xls”或“xlsx”格式文件！",closed:function (){self.location.href="${ctx}/affair/affairMonthStudy/list?treeId=${treeId}"}});
			});
		});
		function page(n,s){
			$("#pageNo").val(n);
		...
		function openAddEditDialog(id) {
			var url = "iframe:${ctx}/affair/affairMonthStudy/form?id="+id;
			if (id == null || id == undefined){
				url = "iframe:${ctx}/affair/affairMonthStudy/form";
			}
			top.$.jBox.open(url, "月度学习计划",1000,600,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){
					self.location.href="${ctx}/affair/affairMonthStudy/list?treeId=${treeId}";
				}
			});
        }
        //详情弹窗
		function openDetailDialog(id) {
			var url = "iframe:${ctx}/affair/affairMonthStudy/formDetail?id="+id;
			top.$.jBox.open(url, "月度学习计划",1000,500,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				}
		...
		function openExamineDialog(id) {
			var url = "iframe:${ctx}/affair/affairMonthStudy/examineView?id="+id+"&treeId=${treeId}";
			top.$.jBox.open(url, "月度学习计划审核",1000,500,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){
...
<%--					<li><a href="${ctx}/affair/affairGeneralSituation/list?treeId=${treeId}&parentId=${parentId}">党组织</a></li>--%>
					<shiro:hasPermission name="affair:affairMonthStudy:view"><li class="active"><a href="${ctx}/affair/affairMonthStudy/list?treeId=${treeId}&parentId=${parentId}">两学一做</a></li></shiro:hasPermission>
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/affair/affairMonthStudy/list?treeId=${treeId}">月度学习计划</a></li>
		<li><a href="${ctx}/affair/affairTwoOne?treeId=${treeId}">"两学一做"专题学习</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="affairMonthStudy" action="${ctx}/affair/affairMonthStudy/list?treeId=${treeId}" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<%--<input id="fileName" name="fileName" type="hidden" value="月度学习计划表.xlsx"/>--%>
		<ul class="ul-form">
			<li><label>计划名称：</label>
			...
			<li><label>创建时间：</label>
				<input name="startDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${affairMonthStudy.startDate}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
                --
                <input name="endDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
                       value="<fmt:formatDate value="${affairMonthStudy.endDate}" pattern="yyyy-MM-dd"/>"
                       onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
			</li>
		</ul>
		<ul class="ul-form2">
			<li class="btns"><input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/></li>
			<shiro:hasPermission name="affair:affairMonthStudy:edit">
				<li class="btns"><input id="btn" class="btn btn-primary" type="button" onclick="openAddEditDialog()" value="添加"/></li>
				<li class="btns"><input id="del" class="btn btn-primary" type="button" value="删除" onclick="deleteByIds('${ctx}/affair/affairMonthStudy/deleteByIds','checkAll','myCheckBox')"/></li>
			</shiro:hasPermission>
				<%--<li class="btns"><input id="btnImport" class="btn btn-primary" type="button" value="导入"/></li>--%>
				<%--<li class="btns"><input id="export" class="btn btn-primary" type="button" value="导出"/></li>--%>
			<li class="btns"><input id="print" class="btn btn-primary" type="button" value="打印" /></li>
			<li class="btns"><input  class="btn btn-primary" type="button" value="重置" onclick="window.location.href='${ctx}/affair/affairMonthStudy/list?treeId=${treeId}'"/></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>创建时间</th>
				<th>审核状态</th>
				<shiro:hasPermission name="affair:affairMonthStudy:edit"><th id="handleTh">操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="affairMonthStudy" varStatus="status">
			<tr>
				<td>
					<input style='margin-left:12px' type='checkbox' name="myCheckBox" value="${affairMonthStudy.id}"/>
				</td>
                <td>${(page.pageNo-1)*page.pageSize+status.index+1}</td>
				<td>

						${affairMonthStudy.title}
				</td>
				<td>
					${affairMonthStudy.partyOrganization}
				</td>
				<%--<td>
					${affairMonthStudy.content}
				</td>--%>
				<td>
					<fmt:formatDate value="${affairMonthStudy.createDate}" pattern="yyyy-MM-dd"/>
				</td>
				<td>
					${fns:getDictLabel(affairMonthStudy.examineStatus, 'affair_party_report_status', '')}
				</td>
				<shiro:hasPermission name="affair:affairMonthStudy:edit"><td class="handleTd">
					<a href="javascript:void(0)" onclick="openDetailDialog('${affairMonthStudy.id}')">查看
					</a>
                    <c:if test="${affairMonthStudy.createBy.id == fns:getUser().id}">
                        <a href="javascript:void(0)" onclick="openAddEditDialog('${affairMonthStudy.id}')">修改</a>
                        <a href="${ctx}/affair/affairMonthStudy/delete?id=${affairMonthStudy.id}&treeId=${treeId}"
                           onclick="return confirmx('确认要删除该月度学习计划吗？', this.href)">删除</a>
                    </c:if>
					<c:choose>
						<%--只有创建人能上报  并且是非 同意状态下--%>
						<c:when test="${affairMonthStudy.createBy.id == fns:getUser().id}">
							<c:if test="${affairMonthStudy.examineStatus eq '1'}">
								<a href="${ctx}/affair/affairMonthStudy/report?id=${affairMonthStudy.id}&treeId=${treeId}"
								   onclick="return confirmx('确认要上报该月度学习计划吗？', this.href)">上报</a>
							</c:if>
						</c:when>
						<c:when test="${fns:getUser().id eq '6eb08ded66314165a692f76d8f660abb'|| fns:getUser().id eq '04f9b5355e054b40b7dc7e2a202dc0c3' ||
						 fns:getUser().id eq '8c95559336ab47c688e78b9d4bd04984' || fns:getUser().id eq '630ee4f59967404488d3a3f1fdec8381' || fns:getUser().id eq 'b016b34a09bd45349b3e1ecc5fde9c5d'||
						  ...
						  fns:getUser().id eq '6502f47ca7bf45539a848135ee3c6bc3' || fns:getUser().id eq 'fcaf9421273643d587baaa88735fe661' || fns:getUser().id eq 'ff371386d33a407b9c1e652b613de2bc'}">
							<c:if test="${affairMonthStudy.examineStatus != '1'}">
								<a href="javascript:" onclick="openExamineDialog('${affairMonthStudy.id}')">审核</a>
							</c:if>
						</c:when>
						<c:otherwise>
						</c:otherwise>
