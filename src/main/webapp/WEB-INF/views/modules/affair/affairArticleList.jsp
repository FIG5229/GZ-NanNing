        ...
        function openDialog(id,category) {
            top.$.jBox.open("iframe:${ctx}/affair/article/shenHeDialog?id="+id,category,800,420,{
                ...
                closed:function (){
                    ...
                    self.location.href="${ctx}/affair/article/?repage&category.id=${article.category.id}&delFlag=2";
                }
            });
        };
    </script>
...
<ul class="nav nav-tabs">
    <li class="active"><a href="${ctx}/affair/article/?category.id=${article.category.id}">文章列表</a></li>
...
<form:form id="searchForm" modelAttribute="article" action="${ctx}/affair/article/" method="post" class="breadcrumb form-search">
    <input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
    <input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
    <label>栏目：</label><sys:treeselect id="category" name="category.id" value="${article.category.id}" labelName="category.name"
...
<%--                                   onclick="return confirmx('确认要发布该文章吗？', this.href)">发布</a>--%>
                            ...
                            <c:if test="${article.createBy.id == fns:getUser().id}">
                                <a href="${ctx}/affair/article/form?id=${article.id}">修改</a>
                                <a href="${ctx}/affair/article/delete?id=${article.id}&delFlag=1&categoryId=${article.category.id}"
                                   onclick="return confirmx('确认要删除该文章吗？', this.href)">删除</a>
                            </c:if>
                        </shiro:hasPermission>
            ...
            <c:if test="${article.category.id == '127650291835482991a8b19ddd29029c' || article.category.id == 'f72a5b908848442cb5606c32c65d4632' ||
                ...
                <c:if test="${article.delFlag == '2'}">
                    <td>
                        ...
                        <c:if test="${article.checkType == '4' || article.checkType == '5' || article.checkType == '6'}">
                            <a href="${ctx}/affair/article/report?id=${article.id}"
                               onclick="return confirmx('确认要上报该文章吗？', this.href)">上报</a>
                        </c:if>
                    </td>
                </c:if>
            </c:if>
