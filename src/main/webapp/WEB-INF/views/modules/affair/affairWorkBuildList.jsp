...
<head>
	...
	<script type="text/javascript">
		...
		function openAddEditDialog(id) {
			var url = "iframe:${ctx}/affair/affairWorkBuild/form?id="+id;
			if (id == null || id == undefined){
				url = "iframe:${ctx}/affair/affairWorkBuild/form";
			}
			top.$.jBox.open(url, "新建党建工作",1000,650,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){
					self.location.href = "${ctx}/affair/affairWorkBuild/list?treeId=${treeId}";
				}
			});
        }
        //详情弹窗
		function openDetailDialog(id) {
			var url = "iframe:${ctx}/affair/affairWorkBuild/formDetail?id="+id;
			top.$.jBox.open(url, "党建工作领导小组工作详情",1000,400,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				}
		...
		function openExamineDialog(id) {
			var url = "iframe:${ctx}/affair/affairWorkBuild/shenHeDialog?id="+id+"&treeId=${treeId}";
...
<body>
	...
	<ul class="nav nav-tabs">
		<c:choose>
			...
			<c:otherwise>
				...
				<li class="active"><a href="${ctx}/affair/affairWorkBuild/list?treeId=${treeId}">党建工作领导小组工作</a></li>
	...
	<form:form id="searchForm" modelAttribute="affairWorkBuild" action="${ctx}/affair/affairWorkBuild/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<shiro:hasPermission name="affair:affairWorkBuild:edit">
				...
				<li class="btns"><input id="del" class="btn btn-primary" type="button" value="删除" onclick="deleteByIds('${ctx}/affair/affairWorkBuild/deleteByIds','checkAll','myCheckBox')"/></li>
			</shiro:hasPermission>
			<li class="btns"><input id="print" class="btn btn-primary" type="button" value="打印"/></li>
			<li class="btns"><input class="btn btn-primary" type="button" value="重置"
									onclick="window.location.href='${ctx}/affair/affairWorkBuild/list?treeId=${treeId}'"/></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>标题</th>
				<th>审核状态</th>
				<shiro:hasPermission name="affair:affairWorkBuild:edit"><th id="handleTh">操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="affairWorkBuild" varStatus="status">
			<tr>
				<td>
					<input style='margin-left:12px' type='checkbox' name="myCheckBox" value="${affairWorkBuild.id}"/>
				</td>
				<td>
						${(page.pageNo-1)*page.pageSize+status.index+1}
				</td>
				<td>
						${affairWorkBuild.partyOrganization}
				</td>
				<td>
						${affairWorkBuild.title}
				</td>
				<td>
						${fns:getDictLabel(affairWorkBuild.status, 'affair_party_report_status', '')}
				</td>
				<td class="handleTd">

				<a href="javascript:void(0)" onclick="openDetailDialog('${affairWorkBuild.id}')">查看</a>
				<shiro:hasPermission name="affair:affairWorkBuild:edit">
    				<a href="javascript:void(0)" onclick="openAddEditDialog('${affairWorkBuild.id}')">修改</a>
					<a href="${ctx}/affair/affairWorkBuild/delete?id=${affairWorkBuild.id}" onclick="return confirmx('确认要删除该党建工作吗？', this.href)">删除</a>
					<c:choose>
						<%--只有创建人能上报  并且是非 同意状态下--%>
						<c:when test="${affairWorkBuild.createBy.id == fns:getUser().id}">
							<c:if test="${ affairWorkBuild.status eq '1'}">
								<a href="${ctx}/affair/affairWorkBuild/report?id=${affairWorkBuild.id}&treeId=${treeId}"
						...
						<c:when test="${fns:getUser().id eq 'a699d2e49a3443739e451e294fb66410' ||
							...
							<c:if test="${affairWorkBuild.status eq '2'}">
								<a href="javascript:" onclick="openExamineDialog('${affairWorkBuild.id}')">审核</a>
							</c:if>
						</c:when>
						<c:otherwise>
						</c:otherwise>
