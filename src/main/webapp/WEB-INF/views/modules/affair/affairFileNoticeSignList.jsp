		...
		<div id="sign">
			...
			<div class="controls" style="margin-right: 30px;height: 120px;overflow-y:auto">
				<table id="contentTable" class="table table-striped table-bordered table-condensed" style="margin-left: 37px;width: 635px;">
					...
					<tbody id="affairFileNoticeSignList">
					<c:forEach items="${signList}" var="affairFileNoticeSign" varStatus="status">
						<tr>
							<td>
								${status.index+1}
							</td>
							<td>
								${affairFileNoticeSign.unit}
							</td>
							<td>
								<fmt:formatDate value="${affairFileNoticeSign.date}" pattern="yyyy-MM-dd HH:ss:mm"/>
							</td>
						</tr>
					</c:forEach>
					</tbody>
...
<%--			<input type="button" class="btn btn-primary" value="一键催办" onclick="window.location.href='${ctx}/affair/affairFileNoticeSign/oneKeyUrge?noticeId=${noticeId}'">--%>
...
<%--						<th width="25%" colspan="2">催办</th>--%>
					...
					<c:forEach items="${notSignList}" var="affairFileNoticeSign" varStatus="status">
						<tr>
							<td>
							...
							<td>
									${affairFileNoticeSign.urge}
							</td>
						<%--	<td>
									<button><a href="${ctx}/affair/affairFileNoticeSign/urge?id=${affairFileNoticeSign.id}">催办</a></button>
							</td>--%>
						</tr>
					</c:forEach>
					</tbody>
