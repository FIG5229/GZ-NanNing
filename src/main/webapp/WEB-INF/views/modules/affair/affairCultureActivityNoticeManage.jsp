...
<head>
	...
	<script type="text/javascript">
		...
		function openAddEditDialog(id) {
			var url = "iframe:${ctx}/affair/affairFileNotice/form?id="+id+"&flag=4";
			if (id == null || id == undefined){
				url = "iframe:${ctx}/affair/affairFileNotice/form?flag=4";
			}
			top.$.jBox.open(url, "制度和通知通报",1000,800,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){
					self.location.href="${ctx}/affair/affairFileNotice/manageList?flag=4";
				}
			});
		};
		//详情弹窗
		function openDetailDialog(id) {
			var url = "iframe:${ctx}/affair/affairFileNotice/formDetail?id="+id+"&flag=4";
			top.$.jBox.open(url, "制度和通知通报",1000,700,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				}
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/affair/affairFileNotice?flag=4">查询</a></li>
		<shiro:hasPermission name="affair:affairFileNotice:manage"><li class="active"><a href="${ctx}/affair/affairFileNotice/manageList?flag=4">管理</a></li></shiro:hasPermission>
	</ul>
	...
	<form:form id="searchForm" modelAttribute="affairFileNotice" action="${ctx}/affair/affairFileNotice/manageList" method="post" class="breadcrumb form-search">
		...
		<input id="flag" name="flag" type="hidden" value="4"/>
		<ul class="ul-form">
			<li><label>标题：</label>
			...
			<li class="btns"><input  class="btn btn-primary" type="button" value="重置" onclick="window.location.href='${ctx}/affair/affairFileNotice/manageList?flag=4'"/></li>
			...
			<tr>
				...
				<shiro:hasPermission name="affair:affairFileNotice:manage"><td class="handleTd">
					<c:if test="${affairFileNotice.createBy.id == fns:getUser().id}">
						...
						<a href="${ctx}/affair/affairFileNotice/delete?id=${affairFileNotice.id}&flag=4"
