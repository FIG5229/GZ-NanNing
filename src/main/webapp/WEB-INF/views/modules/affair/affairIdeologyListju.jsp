...
<body>
	<ul class="nav nav-tabs">
		<c:if test="${fns:getUser().id == '66937439b2124f328d1521968fab06db' || fns:getUser().id == '8a6819768aef40968e8f289842613276'}">
			<li><a href="${ctx}/affair/affairIdeology/jiceng">基层党支部</a></li>
		</c:if>
		<c:if test="${fns:getUser().id == '66937439b2124f328d1521968fab06db' || fns:getUser().id == '8a6819768aef40968e8f289842613276'}">
			<li><a href="${ctx}/affair/affairIdeology/chu">公安处党委</a></li>
		</c:if>
		<li class="active"><a href="${ctx}/affair/affairIdeology/ju">
			<c:choose>
				<c:when test="${fns:getUser().id == '66937439b2124f328d1521968fab06db' || fns:getUser().id == '8a6819768aef40968e8f289842613276'}">
					公安局党委
				</c:when>
				<c:otherwise>基层党支部</c:otherwise>
			</c:choose>
		</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="affairIdeology" action="${ctx}/affair/affairIdeology/jiceng" method="post" class="breadcrumb form-search">
		...
		<input id="classify" name="classify" type="hidden" value="3"/>
		<ul class="ul-form">
		<%--	<li><label>单位：</label>
