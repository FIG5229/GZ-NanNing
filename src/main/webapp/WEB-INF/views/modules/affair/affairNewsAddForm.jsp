			...
			<div class="controls">
				<table id="authorTable" class="table table-striped table-bordered table-condensed">
					...
					<tr>
						...
						<th>姓名</th>
						<th width="10">&nbsp;
&lt;%&ndash;							<input  class="btn btn-primary"  type="button" value="导入" onclick="what()"/>&ndash;%&gt;
								&lt;%&ndash;<input  class="btn btn-primary"  type="button" value="导出" onclick="exportChild('${affairPoliceHome.id}')"/>
                            &ndash;%&gt;
						</th>
					</tr>
					</thead>
...
&lt;%&ndash;				<sys:treeselect id="unit" name="unitId" value="${affairNews.unitId}" labelName="unit" labelValue="${affairNews.unit}"
								title="所属单位" url="/sys/office/treeData?type=2" cssClass="required" allowClear="true" notAllowSelectParent="false" dataMsgRequired="必填信息"/>&ndash;%&gt;
...
&lt;%&ndash;							<input  class="btn btn-primary"  type="button" value="导入" onclick="what()"/>&ndash;%&gt;
				...
				<tr id="affairNewsNameList{{idx}}">
					...
					<td>
						<input id="affairNewsNameList{{idx}}_newsIdNumber" name="affairNewsNameList[{{idx}}].newsIdNumber" type="text" value="{{row.newsIdNumber}}" class="input-small "/>
					</td>
		...
		<div class="control-group">
			...
			<div class="controls">
				<table id="childTable" class="table table-striped table-bordered table-condensed">
					<thead>
					<tr>
						<th class="hide"></th>
						<th>单位</th>
					...
					<shiro:hasPermission name="affair:affairNews:edit">
						...
						<tr>
							<td colspan="46">
								<a href="javascript:;"
								   onclick="addRow('#newsChildList', affairNewsChildrenRowIdx, affairNewsChildrenTpl);affairNewsChildrenRowIdx = affairNewsChildrenRowIdx + 1;"
								   class="btn">新增</a>
							</td>
						</tr>
						</tfoot>
				...
				<script type="text/template" id="affairNewsChildrenTpl">//<!--
				<tr id="newsChildList{{idx}}">
					<td class="hide">
						<input id="newsChildList{{idx}}_id" name="newsChildList[{{idx}}].id" type="hidden" value="{{row.id}}"/>
						<input id="newsChildList{{idx}}_delFlag" name="newsChildList[{{idx}}].delFlag" type="hidden" value="0"/>
						<input id="newsChildList{{idx}}_newsId" name="newsChildList[{{idx}}].newsId" type="hidden" value="{{row.newsId}}"/>
					</td>
					<td>
						<%--<input id="newsChildList{{idx}}_newsUnit" name="newsChildList[{{idx}}].newsUnit" type="text" value="{{row.newsUnit}}" class="input-small " onclick="testOnclick(this)"/>
						--%>
						<sys:treeselect id="newsChildList{{idx}}_newsUnit" name="newsChildList[{{idx}}].newsUnitId"  value="{{row.newsUnitId}}" labelName="newsChildList[{{idx}}].newsUnit" labelValue="{{row.newsUnit}}"
										title="单位" url="/sys/office/treeData?type=2" allowClear="true" notAllowSelectParent="false" dataMsgRequired="必填信息" />
					</td>
					<td>
						<input id="newsChildList{{idx}}_newsName" name="newsChildList[{{idx}}].newsName" type="text" value="{{row.newsName}}" class="input-small "/>
					</td>
					<td>
						<input id="newsChildList{{idx}}_newsIdNumber" name="newsChildList[{{idx}}].newsIdNumber" type="text" value="{{row.newsIdNumber}}" class="input-small "/>
					</td>
					<td>
						<input id="newsChildList{{idx}}_newsAuthor" name="newsChildList[{{idx}}].newsAuthor" type="text" value="{{row.newsAuthor}}" class="input-small "/>
					</td>
					<td>

					<td class="text-center" width="10">
						{{#delBtn}}<span class="close" onclick="delRow(this, '#newsChildList{{idx}}')" title="删除">&times;</span>{{/delBtn}}
					</td>
				</tr>//-->
				</script>
				<script type="text/javascript">
					var affairNewsChildrenRowIdx = 0,
							affairNewsChildrenTpl = $("#affairNewsChildrenTpl").html().replace(/(\/\/\<!\-\-)|(\/\/\-\->)/g, "");
					function initData(data){
						$('#newsChildList').html('');
						for (var i = 0; i < data.length; i++) {
							addRow('#newsChildList', affairNewsChildrenRowIdx, affairNewsChildrenTpl, data[i]);
							affairNewsChildrenRowIdx = affairNewsChildrenRowIdx + 1;
						}
					}
					$(document).ready(function () {
						var data = ${fns:toJson(affairNews.newsChildList)};
						initData(data);
					});
				</script>
			</div>
