...
<head>
	...
	<script type="text/javascript">
		...
		function openAddEditDialog(id) {
			var url = "iframe:${ctx}/affair/affairWorkDoneManage/form?id="+id;
			if (id == null || id == undefined){
				url = "iframe:${ctx}/affair/affairWorkDoneManage/form";
			}
			top.$.jBox.open(url, "工作总结管理录入",1000,650,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){
					self.location.href = "${ctx}/affair/affairWorkDoneManage/list?treeId=${treeId}";
				}
			});
		}
		//详情弹窗
		function openDetailDialog(id) {
			var url = "iframe:${ctx}/affair/affairWorkDoneManage/formDetail?id="+id;
			top.$.jBox.open(url, "工作总结管理详情",1000,400,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				}
		...
		function openCheckDialog(title,id) {
			top.$.jBox.open("iframe:${ctx}/affair/affairWorkDoneManage/shenHeDialog?id="+id, "审核",800,420,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},
...
<body>
	<ul class="nav nav-tabs">
		<c:choose>
			...
			<c:otherwise>
				...
				<shiro:hasPermission name="affair:affairWorkDoneManage:view"><li class="active"><a href="${ctx}/affair/affairWorkDoneManage/list?treeId=${treeId}">工作总结</a></li></shiro:hasPermission>
	...
	<form:form id="searchForm" modelAttribute="affairWorkDoneManage" action="${ctx}/affair/affairWorkDoneManage/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>党组织：</label>
				<sys:treeselect id="partyOrganization" name="partyOrganizationId" value="${affairWorkDoneManage.partyOrganizationId}" labelName="partyOrganization" labelValue="${affairWorkDoneManage.partyOrganization}"
			...
			<shiro:hasPermission name="affair:affairWorkDoneManage:edit">
				<li class="btns"><input id="btn" class="btn btn-primary" type="button" onclick="openAddEditDialog()" value="添加"/></li>
				<li class="btns"><input id="del" class="btn btn-primary" type="button" value="删除" onclick="deleteByIds('${ctx}/affair/affairWorkDoneManage/deleteByIds','checkAll','myCheckBox')"/></li>
			</shiro:hasPermission>
			<li class="btns"><input id="print" class="btn btn-primary" type="button" value="打印"/></li>
			<li class="btns"><input class="btn btn-primary" type="button" value="重置"
									onclick="window.location.href='${ctx}/affair/affairWorkDoneManage/list?treeId=${treeId}'"/></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tr>
			...
			<th>标题</th>
			<th>审核状态</th>
			<shiro:hasPermission name="affair:affairWorkDoneManage:edit"><th id="handleTh">操作</th></shiro:hasPermission>
		</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="affairWorkDoneManage" varStatus="status">
			<tr>
				<td>
					<input style='margin-left:12px' type='checkbox' name="myCheckBox" value="${affairWorkDoneManage.id}"/>
				</td>
				<td>
						${(page.pageNo-1)*page.pageSize+status.index+1}
				</td>
				<td>
						${affairWorkDoneManage.title}
				</td>
				<td>
						${fns:getDictLabel(affairWorkDoneManage.status, 'affair_party_report_status', '')}
				</td>
				<td class="handleTd">

					<a href="javascript:void(0)" onclick="openDetailDialog('${affairWorkDoneManage.id}')">查看</a>
					<shiro:hasPermission name="affair:affairWorkDoneManage:edit">
						<c:if test="${fns:getUser().id eq '6eb08ded66314165a692f76d8f660abb'|| fns:getUser().id eq '04f9b5355e054b40b7dc7e2a202dc0c3' ||
						 fns:getUser().id eq '8c95559336ab47c688e78b9d4bd04984' || fns:getUser().id eq '630ee4f59967404488d3a3f1fdec8381' || fns:getUser().id eq 'b016b34a09bd45349b3e1ecc5fde9c5d'||
						  ...
						  fns:getUser().id eq '6502f47ca7bf45539a848135ee3c6bc3' || fns:getUser().id eq 'fcaf9421273643d587baaa88735fe661' || fns:getUser().id eq 'ff371386d33a407b9c1e652b613de2bc'}">
							<c:if test="${affairWorkDoneManage.status == '2'}">
								<a href="javascript:void(0)" onclick="openCheckDialog('审核','${affairWorkDoneManage.id}')">审核</a>
							</c:if>
						</c:if>
						<c:if test="${affairWorkDoneManage.status == '1'}">
							<a href="${ctx}/affair/affairWorkDoneManage/report?id=${affairWorkDoneManage.id}&treeId=${treeId}">上报</a>
						</c:if>
						<a href="javascript:void(0)" onclick="openAddEditDialog('${affairWorkDoneManage.id}')">修改</a>
						<a href="${ctx}/affair/affairWorkDoneManage/delete?id=${affairWorkDoneManage.id}" onclick="return confirmx('确认要删除该工作总结管理吗？', this.href)">删除</a>
					</shiro:hasPermission>
				</td>
			</tr>
		</c:forEach>
