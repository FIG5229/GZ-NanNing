	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#btnExport").click(
					function(){
						var submit = function (v, h, f) {
							if (v == 'all') {
								$("#searchForm").attr("action","${ctx}/affair/affairPoliceThoughtAnalysis/export?classify=5");
								$("#searchForm").submit();
								$("#searchForm").attr("action","${ctx}/affair/affairPoliceThoughtAnalysis/ju?treeId=${treeId}");
							}
							if (v == 'part') {
								$("#searchForm").attr("action","${ctx}/affair/affairPoliceThoughtAnalysis/export?flag=true&tclassify=5");
								$("#searchForm").submit();
								$("#searchForm").attr("action","${ctx}/affair/affairPoliceThoughtAnalysis/ju?}");
							}
							if (v == 'cancel') {
								$.jBox.tip('已取消');
							}
			...
			$("#btnImport").click(function(){
				top.$.jBox.open("iframe:${ctx}/file/template/download/view?id=affair_affairPoliceThoughtAnalysisJu", "导入",800,520,{itle:"导入数据", buttons:{"关闭":true},
					bottomText:"导入文件不能超过5M，仅允许导入“xls”或“xlsx”格式文件！",closed:function (){self.location.href="${ctx}/affair/affairPoliceThoughtAnalysis/ju"}});
			});
			$("#btnAdd").click(function(){
				top.$.jBox.open("iframe:${ctx}/affair/affairPoliceThoughtAnalysis/form?classify=5", "添加",800,520,{itle:"添加", buttons:{"关闭":true}
...
<ul class="nav nav-tabs">
	...
	<c:if test="${fns:getUser().id == '66937439b2124f328d1521968fab06db'||fns:getUser().id == '南宁局政治部宣传教育处'}">
		<li class="active"><a href="${ctx}/affair/affairPoliceThoughtAnalysis/ju">公安局党委</a></li>
	...
	<c:if test="${fns:getUser().id == '66937439b2124f328d1521968fab06db'||fns:getUser().id == '8a6819768aef40968e8f289842613276' ||
		...
		<li><a href="${ctx}/affair/affairPoliceThoughtAnalysis/jiguanzhibu">局机关党支部</a></li>
...
<form:form id="searchForm" modelAttribute="affairPoliceThoughtAnalysis" action="${ctx}/affair/affairPoliceThoughtAnalysis/ju" method="post" class="breadcrumb form-search">
    ...
    <ul class="ul-form2">
        ...
        <li class="btns"><input id="btnRese" class="btn btn-primary" type="button" value="重置"
                                onclick="window.location.href='${ctx}/affair/affairPoliceThoughtAnalysis/ju'"/></li>
...
<%--			<shiro:hasPermission name="affair:affairPoliceThoughtAnalysis:edit">--%>
                <td class="handleTd">
					...
					<%--	<a href="${ctx}/affair/affairPoliceThoughtAnalysis/report?id=${affairPoliceThoughtAnalysis.id}"
						   onclick="return confirmx('确认要删除该民警思想动态分析吗？', this.href)">上报</a>--%>
					</c:if>
				</td>
<%--            </shiro:hasPermission>--%>
		</tr>
