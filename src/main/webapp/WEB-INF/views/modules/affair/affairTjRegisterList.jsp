...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#export").click(
					function(){
						if (${affairTjRegister.idNumber != '' || affairTjRegister.idNumber != null}){
							var submit = function (v, h, f) {
								if (v == 'all') {
									$("#searchForm").attr("action","${ctx}/affair/affairTjRegister/export?idNumber=${affairTjRegister.idNumber}");
									$("#searchForm").submit();
									$("#searchForm").attr("action","${ctx}/affair/affairTjRegister/list");
								}
								if (v == 'part') {
									$("#searchForm").attr("action","${ctx}/affair/affairTjRegister/export?flag=true&idNumber=${affairTjRegister.idNumber}");
									$("#searchForm").submit();
									$("#searchForm").attr("action","${ctx}/affair/affairTjRegister/list");
								}
								if (v == 'cancel') {
						...
						}else if (${affairTjRegister.partyBranchId != '' || affairTjRegister.partyBranchId != null}){
							var submit = function (v, h, f) {
								if (v == 'all') {
									$("#searchForm").attr("action","${ctx}/affair/affairTjRegister/export?partyBranchId=${affairTjRegister.partyBranchId}");
									$("#searchForm").submit();
									$("#searchForm").attr("action","${ctx}/affair/affairTjRegister/list");
								}
								if (v == 'part') {
									$("#searchForm").attr("action","${ctx}/affair/affairTjRegister/export?flag=true&partyBranchId=${affairTjRegister.partyBranchId}");
									$("#searchForm").submit();
									$("#searchForm").attr("action","${ctx}/affair/affairTjRegister/list");
								}
								if (v == 'cancel') {
			...
			$("#btnImport").click(function(){
				top.$.jBox.open("iframe:${ctx}/file/template/download/view?id=affair_affairTjRegister", "导入",800,520,{itle:"导入数据", buttons:{"关闭":true},
					...
					loaded:function(){$(".jbox-content", top.document).css("overflow-y","hidden");},
...
<body>
	...
	<form:form id="searchForm" modelAttribute="affairTjRegister" action="${ctx}/affair/affairTjRegister/list" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<input id="idnum" name="idnum" type="hidden" value="${affairTjRegister.idNumber}"/>
		<input id="partyid" name="partyid" type="hidden" value="${affairTjRegister.partyBranchId}"/>
...
<%--				<th>注册时间</th>--%>
		...
		<c:forEach items="${page.list}" var="affairTjRegisterInfo" varStatus="status">
			<tr>
				...
				<td>
						${fns:getDictLabel(affairTjRegisterInfo.job, 'affair_tnjob', '')}
				</td>
				<td>
					${affairTjRegisterInfo.remark}
				</td>
				<td class="handleTd">
					<a href="javascript:void(0)" onclick="openDetailDialog('${affairTjRegisterInfo.id}')">查看</a>
					<shiro:hasPermission name="affair:affairTjRegister:edit">
					<%--<c:if test="${affairTjRegister.createBy.id == fns:getUser().id}">--%>
