...
<head>
	...
	<script type="text/javascript">
		...
		function openAddEditDialog(id) {
			var url = "iframe:${ctx}/affair/affairFileNotice/form?id="+id+"&flag=7";
			if (id == null || id == undefined){
				url = "iframe:${ctx}/affair/affairFileNotice/form?flag=7";
			}
			top.$.jBox.open(url, "纪检监察通知通报",1000,800,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){
					self.location.href="${ctx}/affair/affairFileNotice/manageList?flag=7";
				}
			});
		};
		//详情弹窗
		function openDetailDialog(id) {
			var url = "iframe:${ctx}/affair/affairFileNotice/formDetail?id="+id+"&flag=7";
			top.$.jBox.open(url, "纪检监察通知通报",1000,700,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				}
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/affair/affairFileNotice?flag=7">查询</a></li>
		<shiro:hasPermission name="affair:affairFileNotice:manage7"><li class="active"><a href="${ctx}/affair/affairFileNotice/manageList?flag=7">管理</a></li></shiro:hasPermission>
	</ul>
	...
	<form:form id="searchForm" modelAttribute="affairFileNotice" action="${ctx}/affair/affairFileNotice/manageList" method="post" class="breadcrumb form-search">
		...
		<input id="flag" name="flag" type="hidden" value="7"/>
		<ul class="ul-form">
			<li><label>标题：</label>
			...
			<shiro:hasPermission name="affair:affairFileNotice:manage7">
			...
			<li class="btns"><input  class="btn btn-primary" type="button" value="重置" onclick="window.location.href='${ctx}/affair/affairFileNotice/manageList?flag=7'"/></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>签收状态</th>
				<th>是否推送到主页</th>
				<shiro:hasPermission name="affair:affairFileNotice:manage7"><th id="handleTh">操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="affairFileNotice" varStatus="status">
			<tr>
				...
				<shiro:hasPermission name="affair:affairFileNotice:manage7"><td class="handleTd">
					<c:if test="${affairFileNotice.createBy.id == fns:getUser().id}">
						...
						<a href="${ctx}/affair/affairFileNotice/delete?id=${affairFileNotice.id}&flag=7"
