...
<head>
    ...
    <script type="text/javascript">
        ...
        function openDialog(id,category) {
            top.$.jBox.open("iframe:${ctx}/affair/article/shenHeDialog?id="+id,category,800,420,{
                buttons:{"关闭":true},
                loaded:function(h){
                    $(".jbox-content", top.document).css("overflow-y","hidden");
                },
                closed:function (){
                    /*此页面只在审核类型打开  delFlag 写死为2*/
                    self.location.href="${ctx}/affair/article/view?repage&delFlag=2";
                }
            });
        };
    </script>
...
<ul class="nav nav-tabs">
    <li class="active"><a href="${ctx}/affair/article/view?delFlag=2">文章列表</a></li>
    <shiro:hasPermission name="affair:article:edit">
       <%-- <li>
            <a href="<c:url value='${fns:getAdminPath()}/affair/article/form?id=${article.id}&category.id=${article.category.id}'><c:param name='category.name' value='${article.category.name}'/></c:url>">文章添加</a>
        </li>--%>
        <%--<c:choose>
            &lt;%&ndash;管理员管理所有&ndash;%&gt;
            ...
            <c:otherwise>
                &lt;%&ndash;改为管理员管理所有	<c:if test="${fns:getUser().admin}">
                        <li>
                            <a href="<c:url value='${fns:getAdminPath()}/cms/article/form?id=${article.id}&category.id=${article.category.id}'><c:param name='category.name' value='${article.category.name}'/></c:url>">文章添加</a>
                        </li>
                    </c:if>&ndash;%&gt;
...
<form:form id="searchForm" modelAttribute="article" action="${ctx}/affair/article/view" method="post" class="breadcrumb form-search">
    ...
    <%--<sys:treeselect id="category" name="category.id" value="${article.category.id}" labelName="category.name"
                                      ...
                                      title="栏目" url="/cms/category/treeData" module="article" notAllowSelectRoot="false"
                                      cssClass="input-small"/>--%>
    <form:select path="category.id" id="category" class="input-medium">
        <form:option value="" label=""/>
        <form:option value="" label=""/>
        <form:options items="${fns:getDictList('affair_audit_column')}" itemValue="value" itemLabel="label"/>
    </form:select>
    <label>标题：</label><form:input path="title" htmlEscape="false" maxlength="50" class="input-small"/>&nbsp;
    <input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/>&nbsp;&nbsp;
    <label>状态：</label><form:radiobuttons id ="delFlag" onclick="$('#searchForm').submit();" path="delFlag" items="${fns:getDictList('cms_del_flag')}"
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
    ...
    <tr>
        ...
        <th>更新时间</th>
        <th>操作</th>
<%--        <c:if test="${article.category.id == '127650291835482991a8b19ddd29029c' || article.category.id == 'f72a5b908848442cb5606c32c65d4632' ||--%>
<%--                                    article.category.id == '3cac8ac633dd43e2845f6c45b0c9c4ea' || article.category.id == 'd82d42b9d0ab4c34b5405dbb8e398020'}">--%>
        ...
        <tr>
                ...
                <shiro:hasPermission name="affair:article:edit">
                    ...
                    <shiro:hasPermission name="affair:article:edit">
                        ...
                        <shiro:hasPermission name="affair:article:edit">
                            <c:if test="${article.delFlag == 2}">
                                <%--未发布时才显示发布--%>
                                <%--                                <a href="${ctx}/cms/article/push?id=${article.id}&delFlag=0&isRe=true&categoryId=${article.category.id}&isColumnAudi=true"--%>
                                <%--                                   onclick="return confirmx('确认要发布该文章吗？', this.href)">发布</a>--%>
                                <shiro:hasPermission name="affair:article:shenhe">

                                    <c:if test="${article.category.id == '127650291835482991a8b19ddd29029c' || article.category.id == 'f72a5b908848442cb5606c32c65d4632' ||
                                    article.category.id == '3cac8ac633dd43e2845f6c45b0c9c4ea' || article.category.id == 'd82d42b9d0ab4c34b5405dbb8e398020'}">
                                        <%--    <c:if test="${ article.officeExamineId == fns:getUser().id || article.officeDirectorExamineId == fns:getUser().id ||
                                                    article.finalExamineId == fns:getUser().id}">
                                                <a href="javascript:;" onclick="openDialog('${article.id}','${article.category.name}')">审核</a>
                                            </c:if>--%>
                                        <c:choose>
                                            <c:when test="${article.checkType == '1' && article.officeExamineId == fns:getUser().id }">
                                                <c:if test="${article.createBy.id != fns:getUser().id}">
                                                    <a href="${ctx}/affair/article/form?id=${article.id}&isColumnAudi=true">修改</a>
                                                </c:if>
                                                <a href="javascript:;" onclick="openDialog('${article.id}','${article.category.name}')">审核</a>
                                            </c:when>
                                            <c:when test="${article.checkType == '2' && article.officeDirectorExamineId == fns:getUser().id}">
                                                <c:if test="${article.createBy.id != fns:getUser().id}">
                                                    <a href="${ctx}/affair/article/form?id=${article.id}&isColumnAudi=true">修改</a>
                                                </c:if>
                                                <a href="javascript:;" onclick="openDialog('${article.id}','${article.category.name}')">审核</a>
                                            </c:when>
                                            <c:when test="${article.checkType == '3' && article.finalExamineId == fns:getUser().id}">
                                                <c:if test="${article.createBy.id != fns:getUser().id}">
                                                    <a href="${ctx}/affair/article/form?id=${article.id}&isColumnAudi=true">修改</a>
                                                </c:if>
                                                <a href="javascript:;" onclick="openDialog('${article.id}','${article.category.name}')">审核</a>
                                            </c:when>
                                            <c:when test="${article.checkType == '4'}"></c:when>
                                            <c:otherwise></c:otherwise>
                                        </c:choose>
                                    </c:if>
                                </shiro:hasPermission>
                            ...
                            <c:if test="${article.createBy.id == fns:getUser().id}">
                                <a href="${ctx}/affair/article/form?id=${article.id}&isColumnAudi=true">修改</a>
                                <a href="${ctx}/affair/article/delete?id=${article.id}&delFlag=1&categoryId=${article.category.id}&isColumnAudi=true"
                                   onclick="return confirmx('确认要删除该文章吗？', this.href)">删除</a>
                            </c:if>
                        </shiro:hasPermission>
            ...
            <td>
                <c:if test="${article.category.id == '127650291835482991a8b19ddd29029c' || article.category.id == 'f72a5b908848442cb5606c32c65d4632' ||
                                    article.category.id == '3cac8ac633dd43e2845f6c45b0c9c4ea' || article.category.id == 'd82d42b9d0ab4c34b5405dbb8e398020'}">
                    <c:choose>
                        <c:when test="${article.checkType == '1'}">办公室审核</c:when>
                        <c:when test="${article.checkType == '2'}">办公室主任审核</c:when>
                        <c:when test="${article.checkType == '3'}">局领导审核</c:when>
                        <c:when test="${article.checkType == '4'}">办公室审核不通过</c:when>
                        <c:when test="${article.checkType == '5'}">办公室主任审核不通过</c:when>
                        <c:when test="${article.checkType == '6'}">局领导审核不通过</c:when>
                        <c:when test="${article.checkType == '0'}">审核通过</c:when>
                        <c:otherwise></c:otherwise>
                    </c:choose>
                </c:if>
            </td>
