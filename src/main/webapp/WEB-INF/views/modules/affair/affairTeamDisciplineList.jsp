	...
	<script type="text/javascript">
		...
		function openForm(url) {
			top.$.jBox.open("iframe:"+url, "纪律作风教育整顿",800,520,{
				...
				},closed:function (){self.location.href="${ctx}/affair/affairTeamDiscipline"}
			});
		}
		//详情弹窗
		function openDetailDialog(id) {
			var url = "iframe:${ctx}/affair/affairTeamDiscipline/formDetail?id="+id;
			top.$.jBox.open(url, "纪律作风教育整顿详情",800,500,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				}
		...
		function signByIds() {
			...
			if (signIds.length > 0) {
				$.ajax({
					type: "post",
					url: "${ctx}/affair/affairTeamDisciplineSign/sign",
					data: {ids: signIds},
					dataType: "json",
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/affair/affairTeamDiscipline/">纪律作风教育整顿</a></li>
	...
	<form:form id="searchForm" modelAttribute="affairTeamDiscipline" action="${ctx}/affair/affairTeamDiscipline/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form2">
			...
			<%--<shiro:hasPermission name="affair:affairTeamDiscipline:add">
			...
			<li class="btns"><input class="btn btn-primary" type="button" value="重置"
									onclick="window.location.href='${ctx}/affair/affairTeamDiscipline'"/></li>
					...
					<c:choose>
						...
						<c:otherwise>
							<tr>
								...
								<td>
									<c:choose>
										<c:when test="${affairTeamDiscipline.signStatus == '1'}">
											已签收
										</c:when>
										<c:otherwise>
											未签收
