                ...
                success:function(data){
                    if(data.success==true && data.result.length==1){
                        ...
                        var submit = function (v, h, f) {
                            // $("#unit").val(data.result[f.selected].unit);
...
<%--	<ul class="nav nav-tabs">--%>
		<%--<li><a href="${ctx}/affair/affairLeaguePunishment/">团内惩罚列表</a></li>--%>
		<%--<li class="active"><a href="${ctx}/affair/affairLeaguePunishment/form?id=${affairLeaguePunishment.id}">团内惩罚<shiro:hasPermission name="affair:affairLeaguePunishment:edit">${not empty affairLeaguePunishment.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="affair:affairLeaguePunishment:edit">查看</shiro:lacksPermission></a></li>--%>
<%--	</ul>--%>
<br/>
	<form:form id="inputForm" modelAttribute="affairLeaguePunishment" action="${ctx}/affair/affairLeaguePunishment/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<sys:message content="${message}"/>		
		<div class="control-group">
			<label class="control-label">处分时间：</label>
			<div class="controls">
				<input name="date" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate required"
					value="<fmt:formatDate value="${affairLeaguePunishment.date}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">处分时所在单位：</label>
			<div class="controls">
				<sys:treeselect id="unit" name="unitId" value="${affairLeaguePunishment.unitId}" labelName="unit" labelValue="${affairLeaguePunishment.unit}"
								title="获奖单位" url="/sys/office/treeData?type=2" cssClass="required" allowClear="true" notAllowSelectParent="false" dataMsgRequired="必填信息"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="punishmentUnit" htmlEscape="false" class="input-xlarge required"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>

		<div class="control-group">
			<label class="control-label">处分名称：</label>
			<div class="controls">
				<form:input path="punishment" htmlEscape="false" class="input-xlarge required"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">处分文号：</label>
			<div class="controls">
		...
		<div class="control-group">
			<label class="control-label">处分级别：</label>
			<div class="controls">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<sys:webuploader input="filePath" type="files" uploadPath="affair/affairLeaguePunishment" selectMultiple="true"/>
			</div>
		</div>

		<div class="form-actions">
			<shiro:hasPermission name="affair:affairLeaguePunishment:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
