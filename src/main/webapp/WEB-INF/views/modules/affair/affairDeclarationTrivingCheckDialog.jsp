...
<body>
<form:form id="inputForm" modelAttribute="affairDeclarationTrivingExcellence" action="${ctx}/affair/affairDeclarationTrivingExcellence/examine" method="post" class="form-horizontal" cssStyle="margin-top: 20px;">
    <form:hidden path="id"/>
    ...
    <c:choose>
        <c:when test="${fns:getUser().id.equals('ff7f9fe2597b40429ded58f8b76a2f65')||fns:getUser().id.equals('45e43fc5465d4dbf849c6ec50609ecf4')}">
            <div style="margin-left: 500px;margin-top: 70px;">
                <input id="btnSubmit" class="btn btn-primary" type="button" value="通过并结束" onclick="setCheckType(3)"/>&nbsp;

                <input id="btnSubmit1" class="btn btn-default" type="button" value="不通过" onclick="setCheckType(4)"/>&nbsp;
            </div>
        </c:when>
        <c:otherwise>
<%--            <div class="controls">--%>
