	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#export").click(
					function(){
						var submit = function (v, h, f) {
							if (v == 'all') {
								$("#searchForm").attr("action","${ctx}/affair/affairCreateBranch/export?treeId=${treeId}");
								$("#searchForm").submit();
								$("#searchForm").attr("action","${ctx}/affair/affairCreateBranch/list?treeId=${treeId}");
							}
							if (v == 'part') {
								$("#searchForm").attr("action","${ctx}/affair/affairCreateBranch/export?flag=true&treeId=${treeId}");
								$("#searchForm").submit();
								$("#searchForm").attr("action","${ctx}/affair/affairCreateBranch/list?treeId=${treeId}");
							}
							if (v == 'cancel') {
			...
			$("#btnImport").click(function(){
				top.$.jBox.open("iframe:${ctx}/file/template/download/view?id=affair_affairCreateBranch", "导入",800,520,{itle:"导入数据", buttons:{"关闭":true},
					bottomText:"导入文件不能超过5M，仅允许导入“xls”或“xlsx”格式文件！",closed:function (){self.location.href="${ctx}/affair/affairCreateBranch/list?treeId=${treeId}"}});
			});
		});
		function page(n,s){
			$("#pageNo").val(n);
		...
		function openAddEditDialog(id) {
			var url = "iframe:${ctx}/affair/affairCreateBranch/form?id="+id;
			if (id == null || id == undefined){
				url = "iframe:${ctx}/affair/affairCreateBranch/form";
			}
			top.$.jBox.open(url, "品牌创建活动", 1000, 600, {
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){
					self.location.href="${ctx}/affair/affairCreateBranch/list?treeId=${treeId}";
				}
			});
		}
		//详情弹窗
		function openDetailDialog(id) {
			var url = "iframe:${ctx}/affair/affairCreateBranch/formDetail?id="+id;
			top.$.jBox.open(url, "品牌创建活动", 900, 650, {
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				}
		...
		function openExamineDialog(id) {
			var url = "iframe:${ctx}/affair/affairCreateBranch/examineView?id="+id+"&treeId=${treeId}";
			top.$.jBox.open(url, "品牌创建审核",1000,500,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){
...
<%--					<li><a href="${ctx}/affair/affairGeneralSituation/list?treeId=${treeId}">党组织</a></li>--%>
					...
					<shiro:hasPermission name="affair:affairCreateBranch:view"><li class="active"><a href="${ctx}/affair/affairCreateBranch/list?treeId=${treeId}&parentId=${parentId}">活动载体</a></li></shiro:hasPermission>
					<c:choose>
	...
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="affair:affairCreateBranch:view"><li class="active"><a href="${ctx}/affair/affairCreateBranch/list?treeId=${treeId}">品牌创建</a></li></shiro:hasPermission>
	...
	<form:form id="searchForm" modelAttribute="affairCreateBranch" action="${ctx}/affair/affairCreateBranch/list?treeId=${treeId}" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<input id="fileName" name="fileName" type="hidden" value="党内创品牌活动表.xlsx"/>
		<ul class="ul-form">
			<li><label>活动名称：</label>
			...
			<li><label>活动时间：</label>
				<input name="startTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${affairCreateBranch.startTime}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
				 --
				<input name="endTime" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					   value="<fmt:formatDate value="${affairCreateBranch.endTime}" pattern="yyyy-MM-dd"/>"
					   onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
			</li>
			...
			<%--<form:input path="joinPerson" htmlEscape="false" class="input-medium"/>--%>
				<sys:treeselect id="joinPerson" name="joinPersonId" value="${affairCreateBranch.joinPersonId}" labelName="joinPerson" labelValue="${affairCreateBranch.joinPerson}"
		...
		<ul class="ul-form2">
			...
			<shiro:hasPermission name="affair:affairCreateBranch:edit">
				<li class="btns"><input id="btn" class="btn btn-primary" type="button" onclick="openAddEditDialog()" value="添加"/></li>
				<li class="btns"><input id="del" class="btn btn-primary" type="button" value="删除" onclick="deleteByIds('${ctx}/affair/affairCreateBranch/deleteByIds','checkAll','myCheckBox')"/></li>
				<li class="btns"><input id="btnImport" class="btn btn-primary" type="button" value="导入"/></li>
			</shiro:hasPermission>
			<li class="btns"><input id="export" class="btn btn-primary" type="button" value="导出"/></li>
			<li class="btns"><input id="print" class="btn btn-primary" type="button" value="打印" /></li>
			<li class="btns"><input  class="btn btn-primary" type="button" value="重置" onclick="window.location.href='${ctx}/affair/affairCreateBranch/list?treeId=${treeId}'"/></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>活动时间</th>
				<th>审核状态</th>
				<shiro:hasPermission name="affair:affairCreateBranch:edit"><th id="handleTh">操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="affairCreateBranch" varStatus="status">
			<tr>
				<td>
					<input style='margin-left:12px' type='checkbox' name="myCheckBox" value="${affairCreateBranch.id}"/>
				</td>
				<td>
						${(page.pageNo-1)*page.pageSize+status.index+1}
				</td>
				<td>
					${affairCreateBranch.name}
			</td>
				<td>
					${affairCreateBranch.partyBranch}
				</td>
				<td>
					${affairCreateBranch.holdUnit}
				</td>
				<td>
					<fmt:formatDate value="${affairCreateBranch.startTime}" pattern="yyyy-MM-dd HH:mm"/>至<fmt:formatDate value="${affairCreateBranch.endTime}" pattern="yyyy-MM-dd HH:mm"/>
				</td>
				<td>
						${fns:getDictLabel(affairCreateBranch.examineStatus, 'affair_party_report_status', '')}
				</td>
				<td class="handleTd">
					<a href="javascript:void(0)" onclick="openDetailDialog('${affairCreateBranch.id}')">查看	</a>
				<shiro:hasPermission name="affair:affairCreateBranch:edit">
                    <c:if test="${affairCreateBranch.createBy.id == fns:getUser().id}">
                        <a href="javascript:void(0)" onclick="openAddEditDialog('${affairCreateBranch.id}')">修改</a>
                        <a href="${ctx}/affair/affairCreateBranch/delete?id=${affairCreateBranch.id}&treeId=${treeId}"
                           onclick="return confirmx('确认要删除该品牌创建活动吗？', this.href)">删除</a>
                    </c:if>
					<c:choose>
						<%--只有创建人能上报  并且是非 同意状态下--%>
						<c:when test="${affairCreateBranch.createBy.id == fns:getUser().id}">
							<c:if test="${affairCreateBranch.examineStatus eq '1'}">
								<a href="${ctx}/affair/affairCreateBranch/report?id=${affairCreateBranch.id}&treeId=${treeId}"
								   onclick="return confirmx('确认要上报该品牌创建活动吗？', this.href)">上报</a>
							</c:if>
						</c:when>
						<c:when test="${fns:getUser().id eq '6eb08ded66314165a692f76d8f660abb'|| fns:getUser().id eq '04f9b5355e054b40b7dc7e2a202dc0c3' ||
						 fns:getUser().id eq '8c95559336ab47c688e78b9d4bd04984' || fns:getUser().id eq '630ee4f59967404488d3a3f1fdec8381' || fns:getUser().id eq 'b016b34a09bd45349b3e1ecc5fde9c5d'||
						  ...
						  fns:getUser().id eq '6502f47ca7bf45539a848135ee3c6bc3' || fns:getUser().id eq 'fcaf9421273643d587baaa88735fe661' || fns:getUser().id eq 'ff371386d33a407b9c1e652b613de2bc'}">
							<c:if test="${affairCreateBranch.examineStatus != '1'}">
								<a href="javascript:" onclick="openExamineDialog('${affairCreateBranch.id}')">审核</a>
							</c:if>
						</c:when>
						<c:otherwise>
						</c:otherwise>
