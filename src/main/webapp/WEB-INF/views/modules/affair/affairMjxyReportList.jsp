	...
	<script type="text/javascript">
					...
					function(){
						var submit = function (v, h, f) {
							if (v == 'all') {
								...
								$("#searchForm").submit();
								$("#searchForm").attr("action","${ctx}/affair/affairMjxyReport/");
							}
							if (v == 'part') {
		...
		function openImportForm(url) {
			var submit = function (v, h, f) {
				if (v == 'cover') {
					...
					top.$.jBox.open("iframe:${ctx}/file/template/personnelBasesdownload/view?id=affair_affairMjxyReport&isCover=1","疗（休）养信息导入",800,520,{title:"疗（休）养信息导入", buttons:{"关闭":true},
				...
				if (v == 'insert') {
					top.$.jBox.open("iframe:${ctx}/file/template/personnelBasesdownload/view?id=affair_affairMjxyReport&isCover=0","疗（休）养信息导入",800,520,{title:"疗（休）养信息导入", buttons:{"关闭":true},
						bottomText:"导入文件不能超过5M，仅允许导入“xls”或“xlsx”格式文件！",
						closed:function (){
							self.location.href=url;
						}});
		...
		function openDialog(id) {
			top.$.jBox.open("iframe:${ctx}/affair/affairMjxyReport/shenHeDialog?id="+id,"休养申报审核",800,420,{
				...
				closed:function (){
					self.location.href="${ctx}/affair/affairMjxyReport?mType=3";
				}
			});
        }
        //详情弹窗
		function openDetailDialog(type,unit,startDate,endDate,place) {
			var height = $(window).height();
			height = height-10;
			var width = $(window).width();
			width = width;
			var url = "iframe:${ctx}/affair/affairMjxyReport/index?mType=1&type="+type+"&unit="+unit+"&startDate="+startDate+"&endDate="+endDate+"&place="+place;
			top.$.jBox.open(url, "民警休养详情",width,height,{
				persistent: true,  //设置点击窗口外不关闭的参数
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
...
<%--					<li class="btns"><input id="del" class="btn btn-primary" type="button" value="删除" onclick="deleteByIds('${ctx}/affair/affairMjxyReport/deleteByIds','checkAll','myCheckBox')"/></li>--%>
					<li class="btns"><input  class="btn btn-primary" type="button" value="导入" onclick="openImportForm('${ctx}/affair/affairMjxyReport?mType=3')"/></li>
				</shiro:hasPermission>
		...
		<form:form id="searchForm" modelAttribute="affairMjxyReport" action="${ctx}/affair/affairMjxyReport/" method="post" class="breadcrumb form-search">
			...
			<ul class="ul-form2">
				...
				<shiro:hasPermission name="affair:affairMjxyReport:edit">
					...
					<li class="btns"><input id="del" class="btn btn-primary" type="button" value="删除" onclick="deleteByIds('${ctx}/affair/affairMjxyReport/deleteByIds','checkAll','myCheckBox')"/></li>
					<li class="btns"><input  class="btn btn-primary" type="button" value="导入" onclick="openImportForm('${ctx}/affair/affairMjxyReport?mType=1')"/></li>
				</shiro:hasPermission>
				<li class="btns"><input id="export" class="btn btn-primary" type="button" value="导出"/></li>
				<li class="btns"><input id="print" class="btn btn-primary" type="button" value="打印" /></li>
				<li class="btns"><input  class="btn btn-primary" type="button" value="重置" onclick="window.location.href='${ctx}/affair/affairMjxyReport/index?mType=1'"/></li>
				<li class="clearfix"></li>
			</ul>
		</form:form>
	</c:when>
		...
		<form:form id="searchForm" modelAttribute="affairMjxyReport" action="${ctx}/affair/affairMjxyReport/" method="post" class="breadcrumb form-search">
			...
			<ul class="ul-form2">
				...
				<shiro:hasPermission name="affair:affairMjxyReport:edit">
					...
					<li class="btns"><input id="del" class="btn btn-primary" type="button" value="删除" onclick="deleteByIds('${ctx}/affair/affairMjxyReport/deleteByIds','checkAll','myCheckBox')"/></li>
					<li class="btns"><input  class="btn btn-primary" type="button" value="导入" onclick="openImportForm('${ctx}/affair/affairMjxyReport?idNumber=${affairMjxyReport.idNumber}')"/></li>
				</shiro:hasPermission>
				<li class="btns"><input id="export" class="btn btn-primary" type="button" value="导出"/></li>
				<li class="btns"><input id="print" class="btn btn-primary" type="button" value="打印" /></li>
				<li class="btns"><input  class="btn btn-primary" type="button" value="重置" onclick="window.location.href='${ctx}/affair/affairMjxyReport/index?idNumber=${affairMjxyReport.idNumber}'"/></li>
				<li class="clearfix"></li>
			</ul>
		</form:form>
	</c:otherwise>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>序号</th>
				<%--姓名身份证号在工会模块中不显示，人员使用时另行添加，不要再复用了--%>
<%--				<th>姓名</th>--%>
<%--				<th>性别</th>--%>
				<th>休养类型</th>
				<th>单位</th>
		...
		<c:forEach items="${page.list}" var="affairMjxyReport" varStatus="status">
			...
			<%--	<td>
				...
				<c:choose>
					<c:when test="${mType eq '3'}">
						<td class="handleTd">
							<a href="javascript:void(0)" onclick="openDetailDialog('${affairMjxyReport.type}','${affairMjxyReport.unit}','<fmt:formatDate value="${affairMjxyReport.startDate}" pattern="yyyy-MM-dd"/>','<fmt:formatDate value="${affairMjxyReport.endDate}" pattern="yyyy-MM-dd"/>','${affairMjxyReport.place}')">查看</a>
						<%--	<c:if test="${affairMjxyReport.createBy.id == fns:getUser().id &&('0'.equals(affairMjxyReport.checkType)||'1'.equals(affairMjxyReport.checkType))}">
								...
								<a href="${ctx}/affair/affairMjxyReport/deleteIndex?type=${affairMjxyReport.type}&unit=${affairMjxyReport.unit}&startDate=${affairMjxyReport.startDate}&endDate=${affairMjxyReport.endDate}&place=${affairMjxyReport.place}&mType=3" onclick="return confirmx('确认要删除该内容吗？', this.href)">删除</a>
					...
					<c:when test="${mType eq '1'}">
						<td class="handleTd">
							<a href="javascript:void(0)" onclick="openDetailDialog('${affairMjxyReport.id}')">查看</a>
							<a href="javascript:void(0)" onclick="openForm('${ctx}/affair/affairMjxyReport/form?id=${affairMjxyReport.id}','${ctx}/affair/affairMjxyReport/index?mType=1')">修改</a>
