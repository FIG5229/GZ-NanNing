...
<head>
	...
	<script type="text/javascript">
		...
		function openAddEditDialog(id) {
			var url = "iframe:${ctx}/affair/affairVolunteerService/form?id="+id;
			if (id == null || id == undefined){
				url = "iframe:${ctx}/affair/affairVolunteerService/form";
			}
			top.$.jBox.open(url, "志愿服务活动",1100,650,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){
					self.location.href="${ctx}/affair/affairVolunteerService/list?treeId=${treeId}";
				}
			});
		}
		//详情弹窗
		function openDetailDialog(id) {
			var url = "iframe:${ctx}/affair/affairVolunteerService/formDetail?id="+id;
			top.$.jBox.open(url, "志愿服务活动",900,650,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				}
		...
		function openExamineDialog(id) {
			var url = "iframe:${ctx}/affair/affairVolunteerService/examineView?id="+id+"&treeId=${treeId}";
			top.$.jBox.open(url, "志愿服务审核",1000,500,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){
...
<body>
	...
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="affair:affairVolunteerService:view"><li class="active"><a href="${ctx}/affair/affairVolunteerService?treeId=${treeId}">志愿服务</a></li></shiro:hasPermission>
	...
	<form:form id="searchForm" modelAttribute="affairVolunteerService" action="${ctx}/affair/affairVolunteerService?treeId=${treeId}" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label class="width120">活动开始时间：</label>
				<input name="startDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${affairVolunteerService.startDate}" pattern="yyyy-MM-dd"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
				 --
				<input name="endDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
					   value="<fmt:formatDate value="${affairVolunteerService.endDate}" pattern="yyyy-MM-dd"/>"
					   onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/>
			</li>
		...
		<ul class="ul-form2">
			...
			<shiro:hasPermission name="affair:affairVolunteerService:edit">
				<li class="btns"><input id="btn" class="btn btn-primary" type="button" onclick="openAddEditDialog()" value="添加"/></li>
				<li class="btns"><input id="del" class="btn btn-primary" type="button" value="删除" onclick="deleteByIds('${ctx}/affair/affairVolunteerService/deleteByIds','checkAll','myCheckBox')"/></li>
			</shiro:hasPermission>
			<li class="btns"><input id="print" class="btn btn-primary" type="button" value="打印" /></li>
			<li class="btns"><input  class="btn btn-primary" type="button" value="重置" onclick="window.location.href='${ctx}/affair/affairVolunteerService/list?treeId=${treeId}'"/></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>活动时间</th>
				<th>审核状态</th>
				<shiro:hasPermission name="affair:affairVolunteerService:edit"><th id="handleTh">操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="affairVolunteerService" varStatus="status">
			<tr>
				<td>
					<input style='margin-left:12px' type='checkbox' name="myCheckBox" value="${affairVolunteerService.id}"/>
				</td>
				<td>
				...
				<td>
						...
						${affairVolunteerService.name}

				</td>
				<td>
					${affairVolunteerService.partyBranch}
				</td>
				<td>
					${affairVolunteerService.holdUnit}
				</td>
				<td>
					${affairVolunteerService.place}
				</td>
				<td>
					<fmt:formatDate value="${affairVolunteerService.startDate}" pattern="yyyy-MM-dd HH:mm"/> 至 <fmt:formatDate value="${affairVolunteerService.endDate}" pattern="yyyy-MM-dd HH:mm"/>
				</td>
				<td>
						${fns:getDictLabel(affairVolunteerService.examineStatus, 'affair_party_report_status', '')}
				</td>
				<td class="handleTd">
					<a href="javascript:void(0)" onclick="openDetailDialog('${affairVolunteerService.id}')">查看</a>
				<shiro:hasPermission name="affair:affairVolunteerService:edit">
                    <c:if test="${affairVolunteerService.createBy.id == fns:getUser().id}">
                        <a href="javascript:void(0)" onclick="openAddEditDialog('${affairVolunteerService.id}')">修改</a>
                        <a href="${ctx}/affair/affairVolunteerService/delete?id=${affairVolunteerService.id}&treeId=${treeId}"
                           onclick="return confirmx('确认要删除该志愿服务活动吗？', this.href)">删除</a>
                    </c:if>
					<c:choose>
						<%--只有创建人能上报  并且是非 同意状态下--%>
						<c:when test="${affairVolunteerService.createBy.id == fns:getUser().id}">
							<c:if test="${affairVolunteerService.examineStatus eq '1'}">
								<a href="${ctx}/affair/affairVolunteerService/report?id=${affairVolunteerService.id}&treeId=${treeId}"
								   onclick="return confirmx('确认要上报该志愿服务活动吗？', this.href)">上报</a>
							</c:if>
						</c:when>
						<c:when test="${fns:getUser().id eq '6eb08ded66314165a692f76d8f660abb'|| fns:getUser().id eq '04f9b5355e054b40b7dc7e2a202dc0c3' ||
						 fns:getUser().id eq '8c95559336ab47c688e78b9d4bd04984' || fns:getUser().id eq '630ee4f59967404488d3a3f1fdec8381' || fns:getUser().id eq 'b016b34a09bd45349b3e1ecc5fde9c5d'||
						  ...
						  fns:getUser().id eq '6502f47ca7bf45539a848135ee3c6bc3' || fns:getUser().id eq 'fcaf9421273643d587baaa88735fe661' || fns:getUser().id eq 'ff371386d33a407b9c1e652b613de2bc'}">
							<c:if test="${affairVolunteerService.examineStatus != '1'}">
								<a href="javascript:" onclick="openExamineDialog('${affairVolunteerService.id}')">审核</a>
							</c:if>
						</c:when>
						<c:otherwise>
						</c:otherwise>
