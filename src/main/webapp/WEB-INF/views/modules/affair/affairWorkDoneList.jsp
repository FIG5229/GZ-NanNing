...
<head>
	...
	<script type="text/javascript">
		...
		function openAddEditDialog(id) {
			var url = "iframe:${ctx}/affair/affairWorkDone/form?id="+id;
			if (id == null || id == undefined){
				url = "iframe:${ctx}/affair/affairWorkDone/form";
			}
			top.$.jBox.open(url, "工作总结录入",1000,650,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){
					self.location.href = "${ctx}/affair/affairWorkDone/list?treeId=${treeId}";
				}
			});
        }
        //详情弹窗
 		function openDetailDialog(id) {
			var url = "iframe:${ctx}/affair/affairWorkDone/formDetail?id="+id;
			top.$.jBox.open(url, "工作总结详情",1000,400,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				}
		...
		function openExamineDialog(id) {
			var url = "iframe:${ctx}/affair/affairWorkDone/shenHeDialog?id="+id+"&treeId=${treeId}";
...
<body>
	...
	<ul class="nav nav-tabs">
		<c:choose>
			<c:when test="${treeId == '2'}">
				...
				<li class="active"><a href="${ctx}/affair/affairWorkDone/list?treeId=${treeId}">工作总结</a></li>
			</c:when>
			<c:otherwise>
	...
	<form:form id="searchForm" modelAttribute="affairWorkDone" action="${ctx}/affair/affairWorkDone/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li class="btns">
				...
				<shiro:hasPermission name="affair:affairWorkDone:edit">
					<li class="btns"><input id="btn" class="btn btn-primary" type="button" onclick="openAddEditDialog()" value="添加"/></li>
					<li class="btns"><input id="del" class="btn btn-primary" type="button" value="删除" onclick="deleteByIds('${ctx}/affair/affairWorkDone/deleteByIds','checkAll','myCheckBox')"/></li>
				</shiro:hasPermission>
				<li class="btns"><input id="print" class="btn btn-primary" type="button" value="打印"/></li>
				<li class="btns"><input class="btn btn-primary" type="button" value="重置"
					onclick="window.location.href='${ctx}/affair/affairWorkDone/list?treeId=${treeId}'"/></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>标题</th>
				<th>审核状态</th>
				<shiro:hasPermission name="affair:affairWorkDone:edit"><th id="handleTh">操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="affairWorkDone" varStatus="status">
			<tr>
				<td>
					<input style='margin-left:12px' type='checkbox' name="myCheckBox" value="${affairWorkDone.id}"/>
				</td>
				<td>
						${(page.pageNo-1)*page.pageSize+status.index+1}
				</td>
				<td>
						${affairWorkDone.partyOrganization}
				</td>
				<td>
					${affairWorkDone.title}
				</td>
				<td>
						${fns:getDictLabel(affairWorkDone.status, 'affair_party_report_status', '')}
				</td>
				<td class="handleTd">

				<a href="javascript:void(0)" onclick="openDetailDialog('${affairWorkDone.id}')">查看</a>
				<shiro:hasPermission name="affair:affairWorkDone:edit">
    				<a href="javascript:void(0)" onclick="openAddEditDialog('${affairWorkDone.id}')">修改</a>
					<a href="${ctx}/affair/affairWorkDone/delete?id=${affairWorkDone.id}" onclick="return confirmx('确认要删除该工作总结吗？', this.href)">删除</a>
					<c:choose>
						<%--只有创建人能上报  并且是非 同意状态下--%>
						<c:when test="${affairWorkDone.createBy.id == fns:getUser().id}">
							<c:if test="${ affairWorkDone.status eq '1'}">
								<a href="${ctx}/affair/affairWorkDone/report?id=${affairWorkDone.id}&treeId=${treeId}"
						...
						<c:when test="${fns:getUser().id eq 'a699d2e49a3443739e451e294fb66410' ||
							...
							<c:if test="${affairWorkDone.status eq '2'}">
								<a href="javascript:" onclick="openExamineDialog('${affairWorkDone.id}')">审核</a>
							</c:if>
						</c:when>
						<c:otherwise>
						</c:otherwise>
