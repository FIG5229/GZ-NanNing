...
<head>
	...
	<script type="text/javascript">
		...
		function openAddEditDialog(id) {
			var url = "iframe:${ctx}/affair/affairFileNotice/form?id="+id+"&flag=1";
			if (id == null || id == undefined){
				url = "iframe:${ctx}/affair/affairFileNotice/form?flag=1";
			}
			top.$.jBox.open(url, "通知通报文件",1000,600,{
				...
				loaded:function(h){
					...
					self.location.href="${ctx}/affair/affairFileNotice/manageList?flag=1";
				}
			});
		};
		//详情弹窗
		function openDetailDialog(id) {
			var url = "iframe:${ctx}/affair/affairFileNotice/formDetail?id="+id+"&flag=1";
			top.$.jBox.open(url, "通知通报文件",1000,600,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/affair/affairFileNotice?flag=1">查询</a></li>
		...
		<shiro:hasPermission name="affair:affairFileNotice:manage"><li class="active"><a href="${ctx}/affair/affairFileNotice/manageList?flag=1">管理</a></li></shiro:hasPermission>
	</ul>
	...
	<form:form id="searchForm" modelAttribute="affairFileNotice" action="${ctx}/affair/affairFileNotice/manageList" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li><label>类型：</label>
				<form:select id="typeSelect" path="type" class="input-medium">
					...
					<form:options items="${fns:getDictList('affair_file_notice_type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			</li>
			<li><label>发布日期：</label>
		...
		<ul class="ul-form2">
			...
			<li class="btns"><input  class="btn btn-primary" type="button" value="重置" onclick="window.location.href='${ctx}/affair/affairFileNotice/manageList?flag=1'"/></li>
...
<%--				<th>是否推送到主页</th>--%>
		...
		<%--		<td>
				...
				<shiro:hasPermission name="affair:affairFileNotice:manage"><td class="handleTd">
					<a onclick="openDetailDialog('${affairFileNotice.id}')">查看</a>
                    <c:if test="${affairFileNotice.createBy.id == fns:getUser().id}">
                        <%-- 一旦发布不可再修改--%>
						<%--北海处纪检信息管理 266  北海处纪检监察室 266 柳州处纪检信息管理 146  柳州处纪检监察室 146 南宁处纪检信息管理 31  南宁处纪检监察室 31--%>
						<%--(fns:getUser().office.id ne '266' && fns:getUser().office.id ne '146' && fns:getUser().office.id ne '31') 11.9 只有局账号能够推送--%>
						<c:if test="${ fns:getUser().company.id eq '1' && fns:getUser().office.id ne '34' && fns:getUser().office.id ne '95' && fns:getUser().office.id ne '156'}">
							<a href="${ctx}/affair/affairFileNotice/pushNotification?id=${affairFileNotice.id}"
							   onclick="return confirmx('确认要推送该通知通报吗？', this.href)">推送到首页</a>
						</c:if>
						<c:if test="${affairFileNotice.status != '2'}">
                            <a onclick="openAddEditDialog('${affairFileNotice.id}')">修改</a>
                        </c:if>
                        <a href="${ctx}/affair/affairFileNotice/delete?id=${affairFileNotice.id}&flag=1"
