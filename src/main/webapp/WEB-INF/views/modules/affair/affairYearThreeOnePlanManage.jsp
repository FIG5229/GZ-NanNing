...
<head>
	...
	<script type="text/javascript">
		...
		function openDialog(id) {
			top.$.jBox.open("iframe:${ctx}/affair/affairYearThreeOnePlan/shenHeDialog?id="+id, "年度“三会一课”计划审核",800,420,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},
				closed:function (){
					self.location.href="${ctx}/affair/affairYearThreeOnePlan/manageList?treeId=${treeId}";
				}
			});
		}
		//详情弹窗
...
<body>
	...
	<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="affair:affairYearThreeOnePlan:manage"><li class="active"><a href="${ctx}/affair/affairYearThreeOnePlan/manageList?treeId=${treeId}">年度“三会一课”计划审核</a></li></shiro:hasPermission>
	...
	<form:form id="searchForm" modelAttribute="affairYearThreeOnePlan" action="${ctx}/affair/affairYearThreeOnePlan/manageList?treeId=${treeId}" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form2">
			...
			<li class="btns"><input  class="btn btn-primary" type="button" value="重置" onclick="window.location.href='${ctx}/affair/affairYearThreeOnePlan/manageList?treeId=${treeId}'"/></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>党组织名称</th>
				<th>审核状态</th>
				<shiro:hasPermission name="affair:affairYearThreeOnePlan:manage"><th id="handleTh">操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="affairYearThreeOnePlan" varStatus="status">
			<tr>
				...
				<td class="handleTd">
					...
					<a href="javascript:void(0)" onclick="openDialog('${affairYearThreeOnePlan.id}')">审核</a>
				</c:if>
				</td>
			</tr>
		</c:forEach>
