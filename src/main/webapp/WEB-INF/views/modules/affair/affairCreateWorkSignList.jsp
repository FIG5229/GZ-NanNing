...
<head>
	...
	<script type="text/javascript">
		...
		function openDialog(url) {
			top.$.jBox.open("iframe:"+url, "创建工作",800,600,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){self.location.href="${ctx}/affair/affairCreateWork/list"}
			});
		}

		function openSignDialog(id) {
			top.$.jBox.open("iframe:${ctx}/affair/affairCreateWork/manage?id="+id, "创建工作",800,600,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){self.location.href="${ctx}/affair/affairCreateWork/list"}
...
<%--		<shiro:hasPermission name="affair:affairEvaluationCriteria:view">
		...
		<%--	<c:choose>
				<c:when test="${userId=='66937439b2124f328d1521968fab06db' || userId == '8a6819768aef40968e8f289842613276'} ">
					<li class="active"><a href="${ctx}/affair/affairCreateWork/sign">创建工作</a></li>
				</c:when>
				<c:when test="${userId == 'e3ac8381fb3247e0b64fd6e3c48bddc1' || userId=='d30e324c8f73492d9b74103374fbc689'|| userId=='d154234ecb35470e84fb95e53726866b'}">
					...
					<li class="active"><a href="${ctx}/affair/affairCreateWork/sign">创建工作审核</a></li>
				</c:when>
				<c:otherwise>
...
<%--			userId == 'a4e40825e3b34c37895ee2ed168f24a0' || userId=='94ccb0bdb21441f997c72268b4d882db'|| userId=='82f8e9ba1cd7446b99c084d5b4b5c038'}">--%>
				<li class="btns">
					<input  class="btn btn-primary"  type="button" value="添加"
							onclick="openDialog('${ctx}/affair/affairCreateWork/form')"/>
				</li>
				<li class="btns">
		...
		<c:forEach items="${page.list}" var="affairCreateWork" varStatus="status">
			<tr>
				...
				<td class="handleTd">
					...
					<shiro:hasPermission name="affair:affairCreateWork:edit">
					<%--	<c:if test="${affairCreateWork.status != '1' && affairCreateWork.createBy != fns:getUser().id}">
							<c:if test="${affairCreateWork.createBy != fns:getUser().id}">
								<a href="javascript:" onclick="openSignDialog('${affairCreateWork.id}')">审核</a>
							</c:if>
						</c:if>--%>
					<%--	<c:if test="${userId=='66937439b2124f328d1521968fab06db' || userId == '8a6819768aef40968e8f289842613276' ||
							userId == 'a4e40825e3b34c37895ee2ed168f24a0' || userId=='94ccb0bdb21441f997c72268b4d882db'|| userId=='82f8e9ba1cd7446b99c084d5b4b5c038'}">--%>
							<a href="${ctx}/affair/affairCreateWork/form?id=${affairCreateWork.id}">修改</a>
