...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			let delFlag = '${article.delFlag}';
			switch (delFlag) {
				case '0':
					$("input[name='delFlag'][value='0']").attr("checked",true);
					break
				case '1':
					$("input[name='delFlag'][value='1']").attr("checked",true);
					break
				case '2':
					$("input[name='delFlag'][value='2']").attr("checked",true);
					break
			}
		});
		...
		function audi() {
			...
			$("#inputForm").attr("action","${ctx}/affair/article/save?isColumnAudi=true");
			$("#inputForm").submit();
			$("#inputForm").attr("action","${ctx}/affair/article/save");
		}
...
<body>
	<ul class="nav nav-tabs">
		<c:choose>
			<c:when test="${isColumnAudi == 'true'}">
				<li><a href="${ctx}/affair/article/view?delFlag=2">文章列表</a></li>
			</c:when>
			<c:otherwise>
				<li><a href="${ctx}/affair/article/?category.id=${article.category.id}">文章列表</a></li>
			</c:otherwise>
		</c:choose>
		<li class="active"><a href="<c:url value='${fns:getAdminPath()}/affair/article/form?id=${article.id}&category.id=${article.category.id}&isColumnAudi=${isColumnAudi}'><c:param name='category.name' value='${article.category.name}'/></c:url>">文章<shiro:hasPermission name="affair:article:edit">${not empty article.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="affair:article:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="article" action="${ctx}/affair/article/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
...
affair:article:research,affair:article:supervision,affair:article:focusNews,affair:article:announcement,affair:article:inspector,affair:article:education">
		...
		<div class="form-actions">
			<shiro:hasPermission name="affair:article:edit">
				<c:choose>
					<c:when test="${isColumnAudi == 'true'}">
						<input id="btnSubmit" class="btn btn-primary" type="button" value="保 存" onclick="audi()"/>
					</c:when>
					<c:otherwise>
