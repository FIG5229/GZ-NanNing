...
<head>
	...
	<script type="text/javascript">
		...
		function openDetailDialog(id,signStatus) {
			...
			top.$.jBox.open(url, "工会通知通报文件",1200,800,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){
					self.location.href="${ctx}/affair/affairFileNotice?flag=2";
				}
			});
		};
	</script>
...
<ul class="nav nav-tabs">
	<li class="active"><a href="${ctx}/affair/affairFileNotice?flag=2">查询</a></li>
	<shiro:hasPermission name="affair:affairFileNotice:manage2"><li><a href="${ctx}/affair/affairFileNotice/manageList?flag=2">管理</a></li></shiro:hasPermission>
</ul>
	...
	<ul class="ul-form">
        ...
        <li><label>通知类别：</label>
            <form:select id="typeSelect" path="type" class="input-medium">
                ...
                <form:options items="${fns:getDictList('affair_gh_notice_type')}" itemLabel="label" itemValue="value"
                              htmlEscape="false"/>
            </form:select>
        </li>
		<li><label>发布日期：</label>
		...
		<li class="btns"><input  class="btn btn-primary" type="button" value="重置" onclick="window.location.href='${ctx}/affair/affairFileNotice?flag=2'"/></li>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
	...
	<c:forEach items="${page.list}" var="affairFileNotice" varStatus="status">
		<tr>
			...
			<td>
				<a onclick="openDetailDialog('${affairFileNotice.id}','${affairFileNotice.signStatus}')">${affairFileNotice.title}</a>
			</td>
            <td>
