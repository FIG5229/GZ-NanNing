...
<head>
	...
	<script type="text/javascript">
		...
		function openDialog(id) {
			top.$.jBox.open("iframe:${ctx}/affair/affairLifeMeet/shenHeDialog?id="+id, "民主（组织）生活会审核",800,420,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},
				closed:function (){
					self.location.href="${ctx}/affair/affairLifeMeet/manageList?treeId=${treeId}";
				}
			});
		}
		//详情弹窗
		function openDetailDialog(id) {
			...
			top.$.jBox.open(url, "民主（组织）生活会",1000,600,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				}
...
<body>
	<ul class="nav nav-tabs">
			<c:choose>
				...
				<c:otherwise>
					...
					<shiro:hasPermission name="affair:affairLifeMeet:view"><li class="active"><a href="${ctx}/affair/affairLifeMeet/list?treeId=${treeId}">组织生活会</a></li></shiro:hasPermission>
	...
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/affair/affairLifeMeet/list?treeId=${treeId}">录入</a></li>
		<shiro:hasPermission name="affair:affairLifeMeet:manage">
			<li class="active"><a href="${ctx}/affair/affairLifeMeet/manageList?treeId=${treeId}">管理</a></li>
		</shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="affairLifeMeet" action="${ctx}/affair/affairLifeMeet/manageList?treeId=${treeId}" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form2">
			...
			<li class="btns"><input  class="btn btn-primary" type="button" value="重置" onclick="window.location.href='${ctx}/affair/affairLifeMeet/manageList?treeId=${treeId}'"/></li>
		...
		<c:forEach items="${page.list}" var="affairLifeMeet" varStatus="status">
			<tr>
				...
				<td class="handleTd">
					...
					<a href="javascript:void(0)" onclick="openDialog('${affairLifeMeet.id}')">审核</a>
					</c:if>
				</td>
			</tr>
		</c:forEach>
