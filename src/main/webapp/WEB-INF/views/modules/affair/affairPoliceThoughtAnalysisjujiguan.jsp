	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#btnExport").click(
					function(){
						var submit = function (v, h, f) {
							if (v == 'all') {
								$("#searchForm").attr("action","${ctx}/affair/affairPoliceThoughtAnalysis/export?classify=4");
								$("#searchForm").submit();
								$("#searchForm").attr("action","${ctx}/affair/affairPoliceThoughtAnalysis/jujiguan?classify=4");
							}
							if (v == 'part') {
								$("#searchForm").attr("action","${ctx}/affair/affairPoliceThoughtAnalysis/export?flag=true&classify=4");
								$("#searchForm").submit();
								$("#searchForm").attr("action","${ctx}/affair/affairPoliceThoughtAnalysis/jujiguan?classify=4");
							}
							if (v == 'cancel') {
			...
			$("#btnImport").click(function(){
				top.$.jBox.open("iframe:${ctx}/file/template/download/view?id=affair_affairPoliceThoughtAnalysisJiGuanDangWei", "导入",800,520,{itle:"导入数据", buttons:{"关闭":true},
					bottomText:"导入文件不能超过5M，仅允许导入“xls”或“xlsx”格式文件！",closed:function (){self.location.href="${ctx}/affair/affairPoliceThoughtAnalysis/jujiguan"}});
			});
			$("#btnAdd").click(function(){
				top.$.jBox.open("iframe:${ctx}/affair/affairPoliceThoughtAnalysis/form?classify=4", "添加",800,520,{itle:"添加", buttons:{"关闭":true}
				});
...
<ul class="nav nav-tabs">
	...
	<c:if test="${fns:getUser().id == '66937439b2124f328d1521968fab06db'||fns:getUser().id == '8a6819768aef40968e8f289842613276'}">
		...
		<li class="active"><a href="${ctx}/affair/affairPoliceThoughtAnalysis/jujiguan">公安局机关党委</a></li>
...
<form:form id="searchForm" modelAttribute="affairPoliceThoughtAnalysis" action="${ctx}/affair/affairPoliceThoughtAnalysis/jujiguan" method="post" class="breadcrumb form-search">
    ...
    <ul class="ul-form2">
        ...
        <li class="btns"><input id="btnRese" class="btn btn-primary" type="button" value="重置"
                                onclick="window.location.href='${ctx}/affair/affairPoliceThoughtAnalysis/jujiguan'"/></li>
