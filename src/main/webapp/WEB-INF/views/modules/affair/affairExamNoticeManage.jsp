...
<head>
	...
	<script type="text/javascript">
		...
		function openAddEditDialog(id) {
			var url = "iframe:${ctx}/affair/affairFileNotice/form?id="+id+"&flag=5";
			if (id == null || id == undefined){
				url = "iframe:${ctx}/affair/affairFileNotice/form?flag=5";
			}
			top.$.jBox.open(url, "绩效通知通报文件",1000,800,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				},closed:function (){
					self.location.href="${ctx}/affair/affairFileNotice/manageList?flag=5";
				}
			});
		};
		//详情弹窗
		function openDetailDialog(id) {
			var url  = "iframe:${ctx}/affair/affairFileNotice/formDetail?id="+id+"&flag=5";
			top.$.jBox.open(url, "绩效通知通报文件",1000,700,{
				buttons:{"关闭":true},
				loaded:function(h){
					$(".jbox-content", top.document).css("overflow-y","hidden");
				}
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/affair/affairFileNotice?flag=5">查询</a></li>
		...
		<shiro:hasPermission name="affair:affairFileNotice:manage"><li class="active"><a href="${ctx}/affair/affairFileNotice/manageList?flag=5">管理</a></li></shiro:hasPermission>
	</ul>
	...
	<form:form id="searchForm" modelAttribute="affairFileNotice" action="${ctx}/affair/affairFileNotice/manageList" method="post" class="breadcrumb form-search">
		...
		<input id="flag" name="flag" type="hidden" value="5"/>
		<ul class="ul-form">
			<li><label>标题：</label>
			...
			<li class="btns"><input  class="btn btn-primary" type="button" value="重置" onclick="window.location.href='${ctx}/affair/affairFileNotice/manageList?flag=5'"/></li>
			...
			<tr>
				...
				<shiro:hasPermission name="affair:affairFileNotice:manage"><td class="handleTd">
					<c:if test="${affairFileNotice.createBy.id == fns:getUser().id}">
						...
						<a href="${ctx}/affair/affairFileNotice/delete?id=${affairFileNotice.id}&flag=5" onclick="return confirmx('确认要删除该通知通报吗？', this.href)">删除</a>
					</c:if>
