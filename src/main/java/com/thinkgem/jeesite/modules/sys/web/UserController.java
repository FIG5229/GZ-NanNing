...
public class UserController extends BaseController {
	...
	public String blank(User user, Model model) {
		return "modules/sys/blank";
	}
    ...
    public String importFile(MultipartFile file, RedirectAttributes redirectAttributes) {
		...
		try {
			...
			for (User user : list){
				try{
                    if (StringUtils.isNotBlank(user.getNo())) {
                        user.setPassword(SystemService.entryptPassword("123456"));
                        BeanValidators.validateWithException(validator, user);
                        systemService.ImportSaveUser(user);
                        successNum++;
                    }
	...
	public String info(User user, HttpServletResponse response, Model model) {
		...
		String dzb = "";
		if (null != affairPartyMemberService.getGroup(currentUser.getNo()) && "" != affairPartyMemberService.getGroup(currentUser.getNo()).getPartyBranch()){
			dzb = affairPartyMemberService.getGroup(currentUser.getNo()).getPartyBranch();
			groupInfo.put("partyGroupDzb", dzb);
		}else{
			groupInfo.put("partyGroupDzb", "");
		}

		String tw = "";
		if (null != affairTjRegisterService.getGroup(currentUser.getNo()) && "" != affairTjRegisterService.getGroup(currentUser.getNo()).getPartyBranch())
		{
			tw = affairTjRegisterService.getGroup(currentUser.getNo()).getPartyBranch();
			groupInfo.put("partyGroupTw", tw);
		}else {
			groupInfo.put("partyGroupTw", "");
		}
	...
	@RequiresPermissions("user")
	@ResponseBody
	@RequestMapping(value = "examTreeData")
